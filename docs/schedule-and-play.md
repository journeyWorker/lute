...
The driving consumer is Stage onboarding
(`examples/onboarding/lute/` in the
product monorepo) — every example below is real output from that project.
...
placements:
  - event: kuhen
    ...
    size: 4
    variants:
      - when: "run.inflow == 'hanae'"
        doc: scenes/kuhen/hanae.lute
      - when: "run.inflow == 'yunah'"
        doc: scenes/kuhen/yunah.lute
```
...
- **Single-doc form** — `doc: <path>` directly on the placement. Always
  ...
  (the design spec's worked `cara-recon` example).
...
- event: errand-open
  ...
  variants:
    - when: "run.inflow == 'eve'"
      doc: scenes/errand-open/eve.lute
      # eve/yunah keep the placement's own at/size (daytime)
    - when: "run.inflow == 'noriko'"
      doc: scenes/errand-open/noriko.lute
      at: d2.evening+0          # this scene is fixed-evening in-fiction
    - when: "run.inflow == 'hanae'"
      doc: scenes/errand-open/hanae.lute
      at: d2.night+0
      size: 4
```
...
```
scenes/<event>/<variant>.lute      # scenes/confinement/hanae.lute
routes/<route>[-<label>].play.yaml
schedule.yaml
```

Canonical node identity is a scene doc's authored `id: <event>-<variant>`
(dsl 0.15.0 §2), so `scenes/confinement/hanae.lute` writes
`id: confinement-hanae` and simply omits `season:`/`episode:` — the frozen-
...
lute play <PROJECT_DIR>
  --state run.inflow=hanae ...      # seed scalars (route selection)
  --fact  "..." ...                 # seed facts
  --script routes/hanae.play.yaml   # route script (choices; may also carry seeds)
...
above are exercised by the design spec's own worked `cara-recon` example and
by `crates/lute-cli/tests/play.rs`'s rewind/drain test suite.

### Transcript

Human format, real output (`--lanes all`, hanae route):

```
── d6.midnight+0 (tick 564) · user · confinement/hanae ──────────────
::background{location="local_mart_indoor" time="midnight" wait=true}
@hanae{emotion="anxious" voiceKey="hanae-0010"}: 안 열려요!!! 어떡해요!!!!
...
error: unexpected argument 'routes/eve.play.yaml' found
```
...
lute play coverage: 12 script(s) replayed
  ✓ routes/eve.play.yaml: clock exhausted (tick 672) (5 scene(s))
  ✓ routes/hanae.play.yaml: clock exhausted (tick 672) (17 scene(s))
  ...
