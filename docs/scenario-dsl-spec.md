...
flat command-record format (`arcia_script_commands`).
...
> **Worked example:** [`examples/marina-s01ep02.lute`](examples/marina-s01ep02.lute) — the real
> content-repo S01EP02 in this format, with `::camera`, the finger-beam `<timeline>` (four
...
:line[elena]{code="0010" emotion="delighted" variant="1" action="sway"}: 매니저. 안녕…

::camera{focus="elena" zoom="@closeUp" duration="0.5" wait="true"}  /* holds → the line waits for the pan */
:line[elena]{code="0020" emotion="neutral" action="lean"}: 그러니까, 매니저. 딱 한 뼘. 두 뼘.
```
...
Everything desugars to the existing `arcia_script_commands` flat records + CEL strings.
...
2. **the `check()` core** — reuses the canonical `lute-core` parser for the authoritative AST and
   the **existing validator validators** for diagnostics (the semantic checks above are mostly already
   implemented as `validator lint`/`validator validate` rules). The LSP server is a thin adapter exposing
...
The expensive part (semantic validation) already exists as `validator`; every surface is an adapter
...
```upstream
::minigame{kind="rhythm" id="marina_service_01" resultKey="service01" wait="true"}
...
3. Land the new front-end (JSX logic + `::camera` + timing attrs) on the existing
   `arcia_script_commands` compiler.
4. validator lint rule set: exhaustiveness, definite-assignment, `::set` schema-binding,
   wait-omission suspicion, `@ref`/asset/character resolution.
5. tree-sitter grammar → editor highlight/fold; then the LSP adapter over `lute-core` + validator.
...
Design converged over an iterative review with a `codex` DSL-critic peer (via `agent swarm`,
