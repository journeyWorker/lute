...
| `into=` run-record sugar — bool (default value) | line 125 (`into="run.metHelpfully"`) |
| `into=` run-record sugar — enum (explicit `value`) | line 129 (`into="run.sofaOutcome" value="warm"`) |
...
| plugin attr `assetKind` id (decomposed `PT.marina_star.0`) | 89 → `assetkinds/poster.yaml` |
...
| `channel` track (fg) | 71 |
| TWO `property` tracks on one subject (`marina.pos`, `marina.opacity`) | 61, 64 |
