# Haven — a prologue in eleven scenes
...
- **`::end` — first, and still the only.**
  ...
  Before Haven, no document under `docs/examples/` had ever terminated a walk. Read
  [T5.1 and T5.5](../../superpowers/notes/2026-07-31-haven-drive-test-findings.md)
...
- **`identity:` — first, and still the only.** Haven's is the one
...
lute test docs/examples/haven --coverage    # 15 rows; three are permanently red
...
`visited("haven.s01ep07") || visited("haven.s01ep08")`, so ep09 unlocks the instant
