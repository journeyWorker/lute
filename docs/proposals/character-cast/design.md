...
The upstream inline compiler overloads a single `:line[...]` bracket to carry three unrelated
concepts, using delimiters, and mis-names the fields. Grounded in code:

- `packages/lute-core/src/modules/scenario/inline/parser.ts:801-808` — `:line[???-marina]` is split
  at the **last hyphen** into `speaker="???"` (the dialog-box label) and `display="marina"` (the
...
- Character *variants* are faked as **separate ids** via an underscore suffix (`eve_child`,
  `cara_formal`), even though the asset system already has a first-class **costume** dimension:
...
- `.../inline/minter.ts` independently re-implements the last-hyphen rule (`resolveBracketCharacter`)
  ...
  prompt standards).
- `:line[???]` with no id (archived marina S01EP01) collapses `speaker=display="???"` → asset lookup
...
Authored usage is widespread: `???-{id}` across eve/marina/eris/cara scenarios and the
`prompts/scenario-formatter.md` / `scenario-standard-writer.md` standards; `eve_child`/`cara_formal`
...
`:line[marina]` is always the stable `characterId` (author-facing). It is written from the
...
marina:
  name: { ko: "비앙카", en: "Marina" }     # canonical display name per language
...
eve:
  ...
  costumeVoiceBank: { child: eve_child }
```
...
cast:
  ...
  kenshi: { costume: default }       # sealed omitted → false (real name)
---
```
...
- `::seal{character="marina"}` → `::set{scene.cast.marina.sealed = true}`
- `::reveal{character="marina"}` → `::set{scene.cast.marina.sealed = false}`
- `::wear{character="marina" costume="casual"}` → `::set{scene.cast.marina.costume = 'casual'}`
...
| Legacy | New | voiceKey |
|---|---|---|
| `:line[???-marina]{code=…}` | `:line[marina]{code=…}` + `cast.marina.sealed: true` (or `::seal`) | unchanged (`marina-…`) |
| `:line[???]` (no id) | requires an explicit id — flag for author fix | n/a |
| `:line[eve_child]{code=…}` | `:line[eve]{code=…}` + `cast.eve.costume: child` + `registry.eve.costumeVoiceBank.child = eve_child` | preserved (`eve_child-…`) |
| `:line[cara_formal]{code=…}` | `:line[cara]{code=…}` + `costume: formal` + `costumeVoiceBank.formal = cara_formal` | preserved (`cara_formal-…`) |
...
profile: story
cast:
  marina: { costume: waitress, sealed: true, maskAs: "은발의 종업원" }
---

## Shot 1.
::auto{character="marina" anchor="center" action="fade-in-up"}
:line[marina]{code="0010" emotion="content"}: 이쪽으로 오세요, 손님.
/* 화면: "은발의 종업원" · 스프라이트: CH.marina.waitress.content.0 · voiceKey: marina-0010 */

## Shot 4.
::reveal{character="marina"}
:line[marina]{code="0110" emotion="delighted"}: 저는 비앙카예요.
/* 화면: "비앙카" (registry) · 이후 모든 라인 자동 전환 */

::wear{character="marina" costume="casual"}
:line[marina]{code="0120" emotion="shy"}: 편한 옷으로 갈아입었어요.
/* 스프라이트: CH.marina.casual.shy.0 · voiceKey: marina-0120 (costume 무관) */
```
...
- Cast plugin id/ownership (`arcia.cast` vs generic `lute.cast`) — decide at proposal time; lute
