...
3. **Declarations as `---` frontmatter, not a `:::meta` fence.** Repo-fit: upstream already lives on
   `---` YAML frontmatter (editorials, chunks, `.md`/`.json` sidecars, validator-validated). `:::meta`
...
<match on="run.choseHelp">           # reads the run-scoped carry from ep02 — "starts from here"
  <when test="$ == true"> :line[elena]{code="..."}: 저번에 도와줘서… </when>
...
**Resolution (operator-confirmed; codex DSL-critic discussion, `agent swarm`):** never carry raw
...
## Audit trail (codex DSL-critic, via `agent swarm`)
