...
**This file is the primary deliverable.** The Haven example is the instrument; this
...
given, and they do not drive the order: the four `DOC-WRONG`s at rank 6 outrank a
`LANGUAGE-GAP` below them, and the log's single most consequential entry class is
*silence*, which is not a verdict at all.
...
1. **`::set` is not typed against the path it writes (T3.2, `TOOL-DEFECT`).** The single
   ...
   `<choice into="run.shedPressure">` without a value gets `E-INTO-VALUE` — *"`value` is
   required for `run.shedPressure` (only a `bool` path defaults to `true`)"* — one
   construct away, in the same compiler run. It never prints `number`: it names the path
   and the single type that may omit a value, which is exactly T3.2's own reading — the
   diagnostic can only exist because the checker knows this path is not a `bool` at that
   moment. The checker knows.
...
5. **"You got through it without X" is unwritable, and the form you reach for completes
   ...
   success.
6. **The documentation errors that stop a search (T7.7, T3.13, T10.1, T8.6 — four, all
   `DOC-WRONG`).** Ranked as a cluster and above a `LANGUAGE-GAP`, because a gap you
...
8. **`::end` is not an ending (T5.5, `DOC-WRONG`; T5.4, `LANGUAGE-GAP`; T5.7,
   `SPEC-WRONG`).** It is `break` with a label — exactly equivalent to falling off the end
   of the command array — so "which nodes are terminals", "does every route reach one" and
   "can a route dead-end" are not unanswered but *unaskable*. Ranked eighth, not higher,
   for a reason worth stating: the finding is a false sentence on the homepage, and the
   condition it names costs an author modelling honesty rather than correctness —
   **everywhere except T5.7, which is the one item in this cluster whose cost is money.**
   Provably-dead content after a terminator is a *warning*, while the same reachability
   walk calls provably-dead gated content an error and emits no artifact at all
   (`E-ARM-DEAD`); so the dead line is reported *and then shipped* — an addressed command
   record in the artifact, an entry in `loc export`, and, re-measured on Haven's shed
   terminal, +1 line and +4 words of the production word count (177/2,302 → 178/2,306) at
   exit 0. That is a correctness-and-invoice cost, not a modelling one, and it is the
   carve-out from the sentence above rather than a qualification of it: the demotion to
   rank 8 holds for the cluster and does not hold for T5.7's half of it. Note also that
   T8.10 and T8.3 relocate the cause — the structural questions are unaskable because of
   the prerequisite grammar (#4), not because of `::end`. Ranked as its own item because
   the *claim about endings* — a set, a polarity — is separately unstatable and reachable
...
**26 of the 111 entries are *worked well*** — the single largest disposition after
...
- **`lute init` produces a working baseline, not a repair job.** It checks clean as
  ...
  `default:`) *and why* — "the compiler reads those instead of guessing from names". Haven's
...
- **The checker's domain enforcement, demonstrated rather than asserted.** T2.3's negative
  ...
  one function both checker and compiler call. Nothing in Haven's vocabulary would have
...
**111 numbered entries. 74 carry one of the seven verdicts.** Recounted heading by heading
from the entries themselves; every per-task tally in this log reproduced exactly, with one
wording note recorded below. The 111th (T10.4) was filed in T10's fix pass, and every
figure in this section and in `docs/examples/haven/README.md` was re-derived after it.

| verdict | count | entries |
|---|---|---|
| `TOOL-DEFECT` | **33** | T1.4, T1.6, T1.10, T2.1, T2.4, T3.2, T3.6, T3.7, T3.9, T3.10, T4.4, T4.5, T4.10, T5.3, T5.6, T6.3, T6.7, T6.10, T7.2, T7.8, T8.2, T8.5, T9.6, T9.8, T9.9, T9.10, T9.11, T9.12, T9.13, T9.15, T9.18, T10.2, T10.4 |
...
measurements** (T1.11, T7.14, T8.11, T8.12, T9.17, T10.3). 74 + 37 = 111.
...
**Read `TOOL-DEFECT` 33 correctly.** It is not 33 broken features. The criterion is "the
...
**28 of the 111 entries carry an explicit self-correction, and 7 of those changed a
...
- **Attempt** — `lute init docs/examples/haven`, then immediately, before
  touching a byte: `lute check-project docs/examples/haven`.
- **Result** — `ok: docs/examples/haven (1 file(s), 0 project-wide warning(s))`,
...
- **Attempt** — read it, then replace it with Haven's vocabulary (brief Step 3).
...
- **Resolution** — Haven's vocabulary is a **member-for-member substitution into
...
- **Attempt** — reproduced from scratch, outside Haven, so this entry reruns
  without the example tree:
  ```console
  $ ./target/debug/lute init /tmp/t14/proj
  $ cp docs/examples/haven/vocabulary.schema.yaml /tmp/t14/proj/vocabulary.schema.yaml
...
- **Attempt** — `lute context docs/examples/haven/scenes/wake.lute`
...
- **Attempt** — `lute context docs/examples/haven/scenes/opening.lute` (37 lines,
...
- **Result — proactive discovery fails.** Searching the shipped website
  ...
  - **`{prefix}`'s derivation is documented nowhere.** That it expands to
    ...
    `character: haven` → `haven.s01ep01` — which is the right way to confirm it
...
- **Result** —
  ```
  "lineId": "haven.s01ep01.vesna_0010"
  "lineId": "haven.s01ep01.vesna_0020"
  "voiceKey": "vesna-0010"
  "voiceKey": "vesna-0020"
  ```
...
- **Attempt** — after the schema swap and the deletion, `lute check-project docs/examples/haven`.
- **Result** — `ok: docs/examples/haven (1 file(s), 0 project-wide warning(s))`,
...
- **The error does exist, one command over.** `lute trace scenes/wake.lute --mock mocks/playthrough.yaml`:
  ```
  docs/examples/haven/scenes/wake.lute:0:0: error [E-TRACE-MOCK-UNDECLARED] `--state run.greeted=…` names a state path not declared in the resolved schema (state-by-typo MUST fail in mocks exactly as in documents, dsl 0.4.0 §4.3, 0.1 §11.1.1)
  ```
...
- **Intent** — `docs/examples/haven` is a project *inside* the `docs/examples`
  project, and acceptance requires both `check-project docs/examples/haven` and
  `check-project docs/examples` to pass. I need to know which manifest governs
  Haven's scenes when the outer root is the one being walked — otherwise the
...
- **Result (a)** — exit 0, closing with
  ...
  among them:
  ```
  ok: docs/examples/haven/scenes/wake.lute (0 warning(s))
  ```
...
- **What this means for Haven, plainly.** Haven's `identity:` block governs only
  when Haven is compiled *as its own root*. Compiled from `docs/examples`, its
  scenes take the outer manifest's templates. Today that is invisible:
  ```console
  $ lute compile docs/examples/haven/scenes/wake.lute --project docs/examples      -o /tmp/a-outer.json
  $ lute compile docs/examples/haven/scenes/wake.lute --project docs/examples/haven -o /tmp/a-own.json
  $ cmp -s /tmp/a-outer.json /tmp/a-own.json && echo identical
  identical
  ```
  Both give `"lineId": "haven.s01ep01.vesna_0010"` and `"voiceKey": "vesna-0010"`.
  ...
  `{prefix}.{speaker}_{code}` / `{speaker}-{code}` (T1.7) — which is what Haven's
  block sets. The day `docs/examples` grows an `identity:` block, every Haven
...
- **Correction of record.** The original probe (c) put a distinctive template on
  ...
  forward; if a task needs Haven's identity templates to apply, it must compile
  with `--project docs/examples/haven`.
...
there prints `[deprecated] 'lute' is now 'upstream lute'` and `0.1.0`. A name
...
identity chain landed exactly on `haven.s01ep01.vesna_0010` first try.
...
- **Result — silence, at every gate I could reach:**
  ```console
  $ lute check docs/examples/haven/scenes/wake.lute --project docs/examples/haven
  ok: … (0 warning(s))                                           # exit 0
  $ lute check … --deny-warnings
  ok: … (0 warning(s))                                           # exit 0
  $ lute check-project docs/examples/haven
  ok: docs/examples/haven (1 file(s), 0 project-wide warning(s))  # exit 0
  ...
  {"kind":"line","addr":"001-0500","role":"dialogue","speaker":"vesna",
   ...
   "action":"go-under","lineId":"haven.s01ep01.vesna_0030","voiceKey":"vesna-0030"}
  ```
...
- **And the checker already warns about this exact shape elsewhere.**
  ...
  ```
  warning [W-INJECT-CONFLICT] `marina` is shown with an explicit `anchor="center"` that `auto-anchor-on-show` would otherwise inject
  ```
...
- **Result — the source. It is read, in two places, and both matter:**
  ...
  attribute. Probe (`/tmp/t2fix/haven/scenes/pose.lute`):
  ```lute
  ## Cold Wake
  ...
  ```
  ```console
  $ ./target/debug/lute compile /tmp/t2fix/haven/scenes/pose.lute \
      --project /tmp/t2fix/haven -o /tmp/t2fix/pose.json          # exit 0
  ```
  ```json
...
- **Attempt** — scratch copy of the example so nothing committed moves —
  `cp -R docs/examples/haven /tmp/t2fix/haven` — plus one added scene,
  `/tmp/t2fix/haven/scenes/stage_state.lute`, verbatim:
  ```lute
  ---
  kind: scene
  character: haven
...
- **Result — clean, at the strictest setting the CLI offers:**
  ```console
  $ ./target/debug/lute check /tmp/t2fix/haven/scenes/stage_state.lute \
      --project /tmp/t2fix/haven --deny-warnings
  ok: /tmp/t2fix/haven/scenes/stage_state.lute (0 warning(s))       # exit 0
  $ ./target/debug/lute check-project /tmp/t2fix/haven
  ok: /tmp/t2fix/haven/scenes/stage_state.lute (0 warning(s))
  ok: /tmp/t2fix/haven/scenes/wake.lute (0 warning(s))
  ok: /tmp/t2fix/haven (2 file(s), 0 project-wide warning(s))       # exit 0
  $ ./target/debug/lute compile /tmp/t2fix/haven/scenes/stage_state.lute \
      --project /tmp/t2fix/haven -o /tmp/t2fix/stage_state.json     # exit 0
  ```
...
`scenes/cryobank.lute`, `haven.s01ep02` — the project's first `<branch>`, its first
...
- **Result** — `check-project docs/examples/haven` → `ok: … (2 file(s), 0 project-wide warning(s))`,
...
- **Result — all three check clean at the strictest setting:**
  ```console
  $ lute check … --deny-warnings
  ok: /tmp/t3/haven/scenes/c_strnum.lute  (0 warning(s))    # exit 0
  ok: /tmp/t3/haven/scenes/c_boolnum.lute (0 warning(s))    # exit 0
  ok: /tmp/t3/haven/scenes/c_paren.lute   (0 warning(s))    # exit 0
  ```
...
- **Why the committed scene uses `::assert` and not `into=` anyway** — not a
  ...
  `run.*` path; what Haven needs downstream is a *relation* between a crew member and a
...
- **Attempt** — `lute context docs/examples/haven/scenes/cryobank.lute`, and `--json`.
...
- **Result** — rejected, correctly, and unusably:
  ...
  `/private/tmp/t3/haven/world.schema.yaml` has parse/frontmatter errors (1 issue(s))
  ```
...
- **Attempt** — `after: 'visited("haven.s01ep01")'` as committed, plus two deliberate
  breakages: a wrong episode (`haven.s01ep99`) and a misspelled character
  (`anso.s01ep01`).
- **Result** — both caught at project level, both with a suggestion:
  ...
  scenes/a_typo.lute:7:1: error [E-CONN-UNKNOWN-NODE] unknown node: no scene resolves to
    key `haven.s01ep99` (`visited`, dsl §2.3/§4.1) — did you mean `haven.s01ep01`?
  scenes/a_bad.lute:7:1:  error [E-CONN-UNKNOWN-NODE] unknown node: no scene resolves to
    key `anso.s01ep01`  (`visited`, dsl §2.3/§4.1) — did you mean `haven.s01ep01`?
  ```
  ...
  $ lute scenario docs/examples/haven
    topological layers:
      layer 0: scene(haven.s01ep01)
      layer 1: scene(haven.s01ep02)
    edges (prerequisite -> dependent) [atom kind(s)]:
      scene(haven.s01ep01) -> scene(haven.s01ep02) [visited]
  ```
...
- **Result — false as written, and this task depends on it being false.**
  `docs/examples/haven/scenes/cryobank.lute` is `kind: scene` (line 2), and four
  `::assert` directives sit inside `<choice>` bodies (lines 18, 19, 24, 25). The
  checker accepts them without qualification —
  `ok: docs/examples/haven/scenes/cryobank.lute (0 warning(s))`, and
...
- **Attempt** — all four, written at once, no hedging, against a scratch copy
  (`/tmp/t4/haven`) with one extra scalar (`run.couplingCut`) so objective 2 had
...
- **Attempt and result** — three runs against the committed file, exit codes exact:
  ...
  $ lute check-project docs/examples/haven      # start="holds(can_halt(nobody))"
  ...
  failed: docs/examples/haven (3 file(s), …)                                # exit 1

  # B — a crew member who cannot, in this story, ever halt the shed
  $ lute check-project docs/examples/haven      # start="holds(can_halt(vesna))"
  ok: docs/examples/haven (3 file(s), 1 project-wide warning(s))            # exit 0

  # C — restored
  $ lute check-project docs/examples/haven      # start="holds(can_halt(toma))"
  ok: docs/examples/haven (3 file(s), 1 project-wide warning(s))            # exit 0
  ```
...
- **Following the referral.** `lute trace` on the quest is genuinely good at the first
  ...
  ```console
  $ lute trace quests/hold-the-spine.lute --project docs/examples/haven
...
- **(b) And when you do supply the conclusion, trace tells you it proves nothing.**
  ```console
  $ lute trace quests/hold-the-spine.lute --project docs/examples/haven --fact "can_halt(toma)"
...
- **(c) The two tools contradict each other about the same word, and trace is the one
  ...
  $ lute trace scenes/cryobank.lute --project docs/examples/haven --fact "awake(toma)"
  trace: … (seeds: 0 paths, 1 facts; 0 selections)
  ## Shot 1. …

  # a different document in the SAME project, same fact — warning
  $ lute trace quests/hold-the-spine.lute --project docs/examples/haven --fact "awake(toma)"
...
- **Result — total silence, and it is quieter than the correct version:**
  ```console
  $ lute check-project /tmp/t4/haven
  ok: /tmp/t4/haven/quests/hold-the-spine.lute (0 warning(s))
  ok: /tmp/t4/haven (3 file(s), 0 project-wide warning(s))                  # exit 0
  ```
...
- **And it is not merely a missing diagnostic — a tool positively asserts the wrong
  ...
  ```console
  $ lute scenario /tmp/t4/haven reach quest:holdTheSpine       # start="false"
    verdict: Unreachable — quest lifecycle proves this quest can never complete
             (E-QUEST-UNREACHABLE), under your declared routes.

  $ lute scenario /tmp/t4/haven reach quest:holdTheSpine       # start="holds(found(toma))"
...
- **Result — three tools, three partial answers, and the union still misses:**
  - **`lute scenario <dir>`** — the quest is not in the graph. Not a node, not a layer,
    ...
    *documented and deliberate* — `scene-graph.md`: "A quest becomes a graph node by
    ...
    edges." Confirmed by adding `after="visited('haven.s01ep02')"`, which puts
...
- **Attempt** — `lute compile docs/examples/haven/quests/hold-the-spine.lute --project docs/examples/haven`.
...
- **Attempt** — `lute scenario docs/examples/haven envelope quest:holdTheSpine`.
...
- With `after="visited('haven.s01ep02')"`, `quest(holdTheSpine)` appears at **layer 2**
  with a real `scene(haven.s01ep02) -> quest(holdTheSpine) [visited]` edge, and the
...
`scenes/bridge.lute` (`haven.s01ep10`, the success terminal) and `scenes/shed.lute`
(`haven.s01ep11`, the failure terminal) — the corpus's **first two `::end`s**. Nothing
...
- **Result** — `ok: docs/examples/haven (5 file(s), …)`, exit 0. Every surface that is
...
- **Attempt** — the first thing anyone types:
  ```console
  $ lute compile docs/examples/haven/scenes/bridge.lute -o /tmp/t5.json
...
- **Intent** — a work with a closed set of endings wants its ending ids closed too. Every
  other value in Haven's content is checked against a declared domain (T1.2, T1.3), so the
...
- **Resolution** — the shipped corpus keeps the two bare `::end{reason}`s. The proxy above
  is not in `docs/examples/haven/` because paying its price for two endings in an
...
- **Verdict** — `LANGUAGE-GAP`, **shape (b)**, for both halves. Nothing in the language
  *means* either claim, so nothing can check either claim; each is reachable only by
  ...
  - **What the proxy costs.** Itemised above, and the first item is the one that makes this
    ...
    join (cost 5, T4.7). The corpus ships neither proxy: `docs/examples/haven/` keeps two
...
- **Attempt** — `lute scenario docs/examples/haven`, `scenario reach`, `--format json`, plus
  a probe scene declaring itself downstream of a terminal.
- **Result** —
  ```
  project root: docs/examples/haven
    topological layers:
      layer 0: scene(haven.s01ep01)
      layer 1: scene(haven.s01ep02)
      layer 2: scene(haven.s01ep10), scene(haven.s01ep11)
    edges (prerequisite -> dependent) [atom kind(s)]:
      scene(haven.s01ep01) -> scene(haven.s01ep02) [visited]
      scene(haven.s01ep02) -> scene(haven.s01ep10) [visited]
      scene(haven.s01ep02) -> scene(haven.s01ep11) [visited]
  ```
  ...
  `scenario reach haven.s01ep10` reports `Reachable` and its prerequisites, nothing about
...
- **The probe that explains why.** A third scene, `after: 'visited("haven.s01ep10")'` —
  ...
  ok: /tmp/t5probe/scenes/after-the-end.lute (0 warning(s))
      layer 3: scene(haven.s01ep12)
      scene(haven.s01ep10) -> scene(haven.s01ep12) [visited]
  ```
  ...
  does anyway, and it means nothing whatsoever about the run. `visited("haven.s01ep10")`
...
- **Result** — the diagnostic is exemplary: one per body, at the first dead node, spec
  ...
  ```console
  $ lute check-project docs/examples/haven          # ok, 5 file(s).  EXIT=0
  ...
  $ lute loc export docs/examples/haven -o /tmp/t5-loc.json
  ...
  $ lute loc report docs/examples/haven | grep shed
  docs/examples/haven/scenes/shed.lute      2      9      …               <-- billed
...
- **The asymmetry.** This is one reachability pass with two severities. Its sibling verdict
  ...
  a scratch scene, both forms, outside Haven:
  ```console
...
- **Resolution** — probe line removed. `check-project docs/examples/haven` back to
  `ok (5 file(s))`.
...
- **Intent** — Vesna at the helm, dead centre. The bridge is the one scene in the prologue
  ...
  `::auto{character="vesna" anchor="center" action="brace"}`. Haven's `anchor` slot is
...
- **Result** — a permanent warning on the finished scene:
  ```
  docs/examples/haven/scenes/bridge.lute:11:34: warning [W-INJECT-CONFLICT] `vesna` is shown with an explicit `anchor="center"` that `auto-anchor-on-show` would otherwise inject
  ```
  ...
  **different** anchor is honoured silently, and writing **none** is silent. So the one
  ...
  `port`, which every other Haven scene writes explicitly, is fine.
...
- **Result** — the walk stops in the right place and the terminator is recorded, but
  ...
  ```console
  $ lute trace docs/examples/haven/scenes/bridge.lute
    ## Shot 1.
      <auto>
...
full reproduction is in `.superpowers/sdd/haven/task-5-report.md`.
...
- **Result** — `E-COMPONENT-BODY` on every leg, exit 1:
  ```
  $ lute check docs/examples/haven/components/purser-interject.component.lute
  ...
  $ lute check docs/examples/haven/scenes/cryobank.lute --project docs/examples/haven
...
- **Resolution** — for Haven, the mitigation the docs prescribe: `purser-interject.component.lute`
...
- **Verdict** — worked well; the rule serves the author here, and I would not change it. The
  ...
  the whole Haven prologue is about would become impossible to audit by reading. Putting the
...
- **Resolution** — every caller spells every argument. For Haven that is one arg on one
...
- **Verdict** — `ERGONOMIC`, with two distinct costs. **No defaults** is the smaller one and
  ...
  (`warm: { type: bool, cel: … }`), and how the `anchor` domain is written in Haven's own
...
- **Control, added during review at `HavenT6Rev`'s request.** The body above puts the param in
...
- **Result — the good half, and it is genuinely good.** `loc export` emits the component's
  ...
  ```json
  { "code": "0020", "file": "docs/examples/haven/components/purser-interject.component.lute",
...
- **Result — `lineId` is `null`, and everything downstream is keyed on `lineId`.**
  ...
  $ lute tag docs/examples/haven/components/purser-interject.component.lute
  lute: already tagged                        # exit 0, file unchanged (diff: no change)

  $ lute compile …/cryobank.lute --project docs/examples/haven --locales bundle.json
  …cryobank.lute:1:1: warning [W-L10N-MISSING] no `ja-JP` text for `haven.s01ep02.purser_0020`
  exit=0
  $ jq -r '.commands[]|select(.kind=="line")|"\(.lineId)\t\(.text)"' cryo-ja.json
  haven.s01ep02.vesna_0010   [ja] Every pod you crack, the Purser reads as load.
  haven.s01ep02.purser_0020  Draw exceeds projection. The schedule advances.   ← SOURCE LANGUAGE
  ```
  The chain: the row exports with no `lineId`; `import` skips it and exits **0**; `compile
  --locales` demands `haven.s01ep02.purser_0020` — a **caller-derived** id that no export row
...
- **Result — the before/after, which is the measurement.** The same Purser beat, one commit
  ...
  haven.s01ep02.purser_0020   cryobank.lute      Allocation notes the draw. The schedule advances.
  haven.s01ep11.purser_0010   shed.lute          Module released. Allocation is satisfied.
  ...
  Haven's null-`lineId` count went 1 → 3 on this task, and the one pre-existing null is a
...
first probe. Four scenes added: `scenes/spine-a.lute` (`haven.s01ep03`, the first shed on
screen and the corpus's second `<timeline>` ever), `scenes/hydroponics.lute`
(`haven.s01ep04`), `scenes/machine-deck.lute` (`haven.s01ep05`), `scenes/stowaway.lute`
(`haven.s01ep06`).
...
- **Attempt** — `lute compile scenes/spine-a.lute --project docs/examples/haven`, then the
...
- **Attempt** — `lute context docs/examples/haven/scenes/spine-a.lute`, which prints
...
- **Attempt** — `language/branch-match-when.md`, §"The `when=` content-line sugar":
  ...
  followed by a worked `@elena{when="run.metHelpfully"}` / `<match on="run.metHelpfully">` pair,
...
- **Result** — `ok`, 0 warnings. And downstream:
  ...
  {"kind":"line","addr":"001-0100","role":"dialogue","speaker":"vesna","text":"",
   "emotion":"hollowed","lineId":"haven.s01ep90.vesna_0010","voiceKey":"vesna-0010"}
  ```
  `lute loc export --format json` emits it as a translatable unit:
  ```json
  {"code":"0010","kind":"line","lineId":"haven.s01ep90.vesna_0010","speaker":"vesna","text":""}
  ```
...
- **Attempt** — `lute trace docs/examples/haven/scenes/hydroponics.lute --project docs/examples/haven --mock …`
...
- **Intent** — write a fourth, fifth, sixth scene of one work. Each one opens by restating that it
  is a scene, that it belongs to Haven, that it is season 1, and which two schemas it imports.
...
- **Attempt** — `lute scenario docs/examples/haven envelope haven.s01ep04`, then the same for
  `haven.s01ep01` and `haven.s01ep06` as controls.
- **Result** — byte-identical output for all three:
  ...
  ```
  `haven.s01ep01` is the root, with no predecessor and no upstream write of any kind;
  `haven.s01ep06` sits behind five scenes and four `::set`s. The tables cannot tell them apart,
...
- **`after:` takes a disjunction and the graph is right.** `stowaway` is reachable from either
  ...
  ```yaml
  after: 'visited("haven.s01ep04") || visited("haven.s01ep05")'
  ```
  ...
  ```
  layer 2: scene(haven.s01ep03), scene(haven.s01ep10), scene(haven.s01ep11)
  layer 3: scene(haven.s01ep04), scene(haven.s01ep05)
  layer 4: scene(haven.s01ep06)
  scene(haven.s01ep04) -> scene(haven.s01ep06) [visited]
  scene(haven.s01ep05) -> scene(haven.s01ep06) [visited]
  ```
...
- **Confirmed stable, not a one-off.** Three ways:
  1. `lute compile docs/examples/property-tracks.lute --project docs/examples` — a second
     document, unrelated to Haven, whose first and only timeline also emits ordinal `1` and no
     `0`.
...
probe. Three scenes added — `scenes/spine-b.lute` (`haven.s01ep07`, the second shed, where the
ep04/ep05 routes are reconciled), `scenes/archive.lute` (`haven.s01ep08`), `scenes/purser.lute`
(`haven.s01ep09`, the confrontation) — both terminals repointed, and a two-line `facts:` block
...
- **Attempt** — the obvious form, and the same predicate the document's own frontmatter is
  ...
  ```lute
  @narrator{code="0055" when="visited('haven.s01ep05')"}: You had both hands on that ring an hour ago.
  <match on="visited('haven.s01ep05')">
...
- **What the language offers instead, enumerated rather than assumed.** The admissible guard
  ...
  write or a fact assertion that only that route performs. Haven has neither by design — and
  **has one by accident**:
...
- **Verdict** — `LANGUAGE-GAP`, **shape (b)**. The story is intact — nothing was cut, and the
  ...
  *what is true now* and has no representation of *how you got here*.** That is a coherent
  ...
  three predicates in a content guard — `visited("haven.s01ep05")` as a read-only query over the
...
- **Attempt** — verbatim, in the first draft:
  ```lute
  <choice id="readTheRecord" label="Go to the archive and find out what it is counting" goto="haven.s01ep08">
  <choice id="goAtItDirect" label="Go forward and say stop" goto="haven.s01ep09">
  <choice id="stayWithNine" label="Stay in the module" goto="haven.s01ep11">
  ```
- **Result** — `ok: docs/examples/haven/scenes/spine-b.lute (0 warning(s))`, exit 0. The same
  run reported the two `E-CEL-PROFILE` errors from T8.1 on the lines above, so the file *was*
  being checked. And the artifact:
  ```console
  $ lute compile …/spine-b.lute --project docs/examples/haven -o /tmp/spineb.json
...
- **What the silence costs, measured.** `into=`/`value=` lower to a real write inside the arm:
  ...
  ```console
  $ lute check probe-fix.lute --project docs/examples/haven
...
- **Attempt and result** — three routes, in the order I reached for them.
  ...
  2. **Have the successor read what the choice wrote.** The natural inversion: `::set` a path in
     ...
     **no state reads at all**:
     ...
     ```
  3. **Exclude the routes not taken.** `after: 'visited("haven.s01ep03") && !visited("haven.s01ep04")'`:
     ```
...
- **The consequence, which is larger than the branch I was writing.** Haven has been described
  through six tasks as forking at ep03 — T7.15 calls ep04/ep05 "the ep03 fork and the ep06 join".
  **It is not a fork.** Both scenes declare `after: 'visited("haven.s01ep03")'`, both become
...
- **Verdict** — `LANGUAGE-GAP`, **both shapes**, and the honest framing takes a paragraph because
  ...
  level — `exclusive: [[haven.s01ep04, haven.s01ep05]]` — naming node sets of which at most one
...
- **Attempt** — `lute scenario docs/examples/haven envelope haven.s01ep09`, and the same for
  `haven.s01ep01`, `haven.s01ep07` and `haven.s01ep10` as controls. ep09 sits behind **eight**
...
- **T7.13 got this wrong, and I inherited the error.** Its text reads: *"`run.shedPressure` is
  ...
  ```console
  $ lute check probe-dead.lute --project docs/examples/haven
...
- **`lute trace` refuses the same selection, on the same document, in the same project.** With
  ...
  ```console
  $ lute trace …/purser.lute --project docs/examples/haven \
...
- **And `lute test` refuses it too, because `lute test` is `trace`-based — which is the
  ...
  ```console
  $ lute test /tmp/t85
  FAIL  /tmp/t85/tests/guard-false-choice.test.yaml  (…/haven/scenes/purser.lute)
        trace refused: invalid mock input
  PASS  /tmp/t85/tests/guard-true-choice.test.yaml   (…/haven/scenes/purser.lute)
...
- **It is not missing information.** The guard is in the artifact, on the option, as a raw CEL
  ...
  {"id":"invalidateTheVoyage","label":"Tell it the voyage is already over",
   "lineId":"haven.s01ep09.theCorrection.invalidateTheVoyage",
...
- **What it printed**, on 13 documents, first run, no probing:
  ...
  topological layers:
    layer 0: scene(haven.s01ep01)
    layer 1: scene(haven.s01ep02)
    layer 2: scene(haven.s01ep03)
    layer 3: scene(haven.s01ep04), scene(haven.s01ep05)
    layer 4: scene(haven.s01ep06)
    layer 5: scene(haven.s01ep07)
    layer 6: scene(haven.s01ep08), scene(haven.s01ep11)
    layer 7: scene(haven.s01ep09)
    layer 8: scene(haven.s01ep10)
  edges (prerequisite -> dependent) [atom kind(s)]:
    scene(haven.s01ep01) -> scene(haven.s01ep02) [visited]
    scene(haven.s01ep02) -> scene(haven.s01ep03) [visited]
    scene(haven.s01ep03) -> scene(haven.s01ep04) [visited]
    scene(haven.s01ep03) -> scene(haven.s01ep05) [visited]
    scene(haven.s01ep04) -> scene(haven.s01ep06) [visited]
    scene(haven.s01ep05) -> scene(haven.s01ep06) [visited]
    scene(haven.s01ep06) -> scene(haven.s01ep07) [visited]
    scene(haven.s01ep07) -> scene(haven.s01ep08) [visited]
    scene(haven.s01ep07) -> scene(haven.s01ep09) [visited]
    scene(haven.s01ep07) -> scene(haven.s01ep11) [visited]
    scene(haven.s01ep08) -> scene(haven.s01ep09) [visited]
    scene(haven.s01ep09) -> scene(haven.s01ep10) [visited]
  ```
...
- **Question 3 — do both endings remain reachable?** *Answerable only as "are both nodes
  reachable", which is a weaker question and is yes.* `reach haven.s01ep10` and
  `reach haven.s01ep11` both report `Reachable`. But the graph has no notion of an ending, so
...
```json
{"speaker":"vensa","lineId":"haven.s01ep96.vensa_0010"}
```
```
speaker    lines   words
…
...
- **T5.8 (`W-INJECT-CONFLICT` on the declared default anchor) — hit three times in three scenes,
  and the shape is now clear.** Haven's `anchor` domain is
...
- **T7.12 (retyped frontmatter) — 18 more boilerplate lines.** Three new scenes, each restating
  `kind: scene`, `character: haven`, `season: 1`, and the same two-schema `uses:`. The corpus's
  ...
  `kind: scene`, `character: haven`, `season: 1`, one `components:`, two `uses:` spellings
  (`wake` imports one schema, the other ten import two), 11 distinct `episode:` lines, and 8
  distinct `after:` formulas — `visited("haven.s01ep03")` is shared by `hydroponics` and
  `machine-deck`, `visited("haven.s01ep07")` by `archive` and `shed`. 66 − 25 = 41. The same rule
...
lines and 9 choices — more than doubling the corpus. (`lute loc report docs/examples/haven`,
...
- **`after:` disjunction, again, and the repointing was genuinely one line each.** `purser.lute`
  landed `after: 'visited("haven.s01ep07") || visited("haven.s01ep08")'` first try, two edges,
...
Final state: 18 `.lute` documents, `check-project docs/examples/haven` ok with 13
project-wide warnings, `lute test docs/examples/haven` 31 passed / 0 failed.
...
- **Resolution** — `after=` on the `<quest>` element:
  ```lute
  <quest id="whoWakes" title="Who Wakes" start="true" after="visited('haven.s01ep01')">
  ```
...
- **Result** — every one lands, on five distinct layers, each with a real edge:
  ```
    layer 1: scene(haven.s01ep02), quest(whoWakes)
    layer 2: scene(haven.s01ep03), quest(falseHeading)
    layer 3: scene(haven.s01ep04), scene(haven.s01ep05), quest(unmoored)
    layer 4: scene(haven.s01ep06), quest(whatVesnaCarries)
    layer 5: scene(haven.s01ep07), quest(manifestGap)
    …
...
- **Attempt** —
  ```lute
  <objective id="arrive" title="Arrive at the bridge" done="visited('haven.s01ep10')"/>
  ```
- **Result** —
  ```
  ...
  error, the same document says `after="visited('haven.s01ep10')"` and it compiles.
...
- **Attempt** — one scratch quest in a copy of the committed project, three
  objectives:
  ```lute
  <quest id="deadQuest" title="Dead" start="true" after="visited('haven.s01ep01')">
...
2. **A test must over-seed to trace one arm.** `haltTheSequence` carries two
   ...
   `archive.lute`'s `after:` is `visited("haven.s01ep07")` and carries no dependency
...
- **Attempt** — `lute trace docs/examples/haven/components/purser-interject.component.lute`.
- **Result** —
  ```
...
`check-project docs/examples` and `lute test docs/examples/haven --coverage`, and
...
here, replaces `docs/examples/haven/README.md`, and registers the one CI surface that
turned out not to exist. Both deferred items are re-derived from scratch below rather
than transcribed from the reports that raised them. T10.4 was added in the fix pass: the
scaffolded README this task overwrote had rotted, and overwriting it without an entry was
a fix that deleted its own measurement.
...
- **The counts, both directions, from the source tree only (no `dist/`, no
  `node_modules/`).** Two criteria, stated because they give different numbers and this
  entry's first draft quoted neither. Under the criterion that matches the page being
  wrong — **an author's state declaration**, which is what `state-model.md`'s block is —
  the working spelling is used in **five shipped example files, ten occurrences**, all
  reached green by `check-project docs/examples`: `affinity-reaction.lute` (a `state:`
  path and a `defs:` param), `choice-persist.lute`, `investigation/world.schema.yaml`
  (three paths), `showcase/schema/base.schema.yaml` (three paths), and
  `showcase/when-is-demo.lute`. Counting **every** `type: { enum: … }` under
  `docs/examples` regardless of construct gives **eleven files, sixteen occurrences**: the
  other six files are the two plugin packs' manifests (`arcia.minigame` and
  `showcase.pack`, each declaring it once in `plugin.yaml` `options:`, once in a `bridge/`
  return shape and once in `state/shapes.yaml`), which are plugin export schemas rather
  than author state. Neither number is six. On the website the working spelling appears in
  `getting-started/build-an-investigation.mdx:46–48`. The broken
...
- **And the two spellings collide on the same declaration.**
  ...
  them was not run.
- **Resolution** — none authorially; Haven declares no `enum` state path (T5.4's proxy
...
- **What was asked** — whether `docs/examples/haven` is covered by the existing
...
- **The check surface is genuinely covered, and nesting is not the problem.** T1.10
  ...
  ```console
  $ ./target/debug/lute check-project docs/examples | grep -c haven
  33
  ```
  All 18 Haven documents are walked by the outer root, and both deliberate
  `W-INJECT-CONFLICT`s and all 13 `W-UNPROVEN-RELATIONAL`s appear in that run. So
  **`docs/examples/haven` needed no registration for `check-project`** — the answer the
...
- **The test surface was covered by nothing.** `lute test` does not appear in
  ...
  `docs/examples/haven/tests/` and 3 under `docs/examples/investigation/tests/` — and no
...
- **Resolution — registered, in both workflows.**
  `cargo run -q -p lute-cli -- test docs/examples`, on the same root and the same
  already-built binary as the neighbouring steps: the `examples` job of
  `.github/workflows/docs.yml`, and the `offline` job of `.github/workflows/test.yml`.
  `lute test` recurses, so one root covers both suites — verified at `34 passed, 0 failed`,
  exit 0, and at exit 1 / `33 passed, 1 failed` when one expected state value is
  perturbed, in both invocation forms (bare `./target/debug/lute test docs/examples` and
  `cargo run -q -p lute-cli -- test docs/examples`).
- **Both workflows, not one, on this entry's own argument.** `docs.yml`'s `paths:` cover
  content and two named crate files, not `crates/**`; `test.yml`'s cover `crates/**` and
  not `docs/**`. In `docs.yml` alone, 34 tests whose every assertion the binary *produces*
  would run when the TESTS moved and never when the RUNNER moved — a PR touching
  `crates/lute-trace` could not fail them. That is this entry's own omission pointed the
  other way, and `test.yml`'s header already states the rule: "`docs.yml` fires when a DOC
  moves, this job fires when the BINARY the doc quotes moves." Both doc scripts run in
  both workflows on exactly that argument; the scenario suites now do too.
  Each step's comment states what a green run does and does not prove, citing T9.18,
...
#### T10.4 — `lute init`'s README names the placeholder scene, and nine tasks of green `check-project` never said so — TOOL-DEFECT

Filed in this task's fix pass, out of order, and the lateness is part of the finding: the
rot was *fixed* in Step 2 — the README this task wrote replaced it — and nothing was
logged. That is exactly the move the capture protocol forbids ("when a beat does not
appear in the artifact, log it before fixing it"), because a fix without an entry deletes
the measurement. Re-derived below from a fresh scaffold and from git, not from memory.

- **Intent** — none authorial. Step 2 replaces `docs/examples/haven/README.md`. Read what
  was there before overwriting it.
- **Result — it was still the scaffolder's, byte for byte.**
  ```console
  $ git show 0050e16^:docs/examples/haven/README.md > /tmp/pre.md
  $ lute init /tmp/t10d/proj --template minimal
  $ diff /tmp/pre.md /tmp/t10d/proj/README.md && echo IDENTICAL
  IDENTICAL
  ```
  Nine task sections of authoring — eighteen documents, both schemas replaced, thirty-one
  test files — and the project's front door was still the template's, naming the scene
  T1.3 calls the placeholder and deleted ("deleted `scenes/opening.lute` per Step 5") in
  two of its seven commands:
  ```sh
  # Check one document:
  ...
  lute trace scenes/opening.lute --mock mocks/playthrough.yaml
  ```
  Both, run on the finished corpus:
  ```console
  $ lute check docs/examples/haven/scenes/opening.lute --project docs/examples/haven
  lute: cannot read docs/examples/haven/scenes/opening.lute: No such file or directory (os error 2)
  # exit 2 — and the same line, same exit, for the `trace` command
  ```
  And `check-project` never mentioned it, at any task boundary. On the finished corpus:
  `ok: docs/examples/haven (18 file(s), 13 project-wide warning(s))`, exit 0.
- **Nothing in this repository read it either, and it was inside a scanned root.**
  `check-project` walks `.lute` documents. `scripts/check-doc-snippets.py` takes `docs` as
  a first-class snippet root over `.md`, excluding only `docs/proposals` and
  `docs/superpowers` — so this file was **in scope** — but its unit of verification is a
  fence whose info word is `lute` (`extract_blocks` returns "the ```lute subset"), and the
  only fence here is a `sh` one. `scripts/check-docs-consistency.py` scans the same tree
  for links and version claims; `scenes/opening.lute` inside a shell fence is neither.
  Three gates over the file's own directory, exit 0 from all three, and the first command
  a human would have copied out of it cannot run.
- **The `init`-side fact that makes this a tool defect rather than an authoring lapse.**
  `lute init` emits its next steps **twice**, and only one copy can rot. To stdout:
  ```console
  $ lute init /tmp/t10f --template minimal
  Next steps:
    lute check-project /tmp/t10f
    lute scenario /tmp/t10f
    lute new scene <name> --dir /tmp/t10f
  ```
  Three commands, every one root-scoped or placeholder-shaped, every path resolved,
  nothing that can go stale. To disk, in `README.md`, it adds the two per-file commands
  above, hard-coded to `scenes/opening.lute` — the one file the scaffold's own last
  content line invites you to replace (*"Edit this scene, then run `lute check-project`"*).
  The rot-free phrasing existed three lines away, in the same tool's own output, at the
  same moment.
- **Resolution** — the file is gone: Step 2 replaced it with a README that names no path
  it does not also list, and every number it quotes is re-derived from the tools. The
  *class* is not closed: the next `lute init` writes the same two lines.
- **Verdict** — `TOOL-DEFECT`, argued against the table, and the argument matters because
  the sibling entry chose differently.
  - Not `DOC-WRONG`: no page on the website is false, and no page edit fixes it. The false
    document is *generated*; correcting one instance leaves the generator intact.
  - Not `DOC-GAP`: nothing was missing to search for.
  - Not `AUTHOR-ERROR`: the criterion is "the docs said so plainly and you missed it".
    Nothing anywhere states that the scaffold README is the author's to maintain, and for
    nine tasks no tool, gate or diagnostic said so either.
  - Not `SPEC-WRONG`: nothing is specified about a scaffold README's contents, so there is
    no design decision to fault and nothing to propose an alternative *spec* for.
  - Not `ERGONOMIC`, and this is the split with **T1.9**, which filed the sibling rot in
    `mocks/playthrough.yaml` — same scaffolder, same deleted scene, same silence — as
    `ERGONOMIC` shading toward silence. There the rotting file is typed data the language
    *could* validate, and T1.9's own remedy is a checker feature (`W-MOCK-ORPHANED`, or a
    `check-project` pass over `mocks/*.yaml`): a closable gap, which reads as a cost. A
    README is prose, and no Lute checker can ever be responsible for it, so no gate closes
    this one. The only place the defect can live is in what `init` chose to write, which is
    a tool being wrong about its own output — `TOOL-DEFECT`'s criterion, and the clause
    under which this log already files three tools that hand an author something they
    should not (T4.10, T9.12, T10.2). Same class of rot as T1.9, a **new artifact class**,
    and the verdicts differ because the remedies live in different places.
- **What it should write instead.** Either drop the two per-file commands from the
  generated README, or write them the way the same block already writes `lute new scene
  <name>`: `lute check scenes/<your-scene>.lute`, a template rather than a claim about a
  file. Strictly better, and free: do not write the README's "Next steps" section at all.
  `init` already printed it — resolved, correct, and unable to rot.

#### T10 summary

Four entries: one `DOC-WRONG` (T10.1), two `TOOL-DEFECT`s (T10.2, T10.4), and one closed
infrastructure gap carrying no verdict (T10.3). No content was written, so there is no
`LANGUAGE-GAP` or `ERGONOMIC` here by construction — this task authored a README and two
CI steps, not a scene.
...
the correct spelling exists in five example files' state declarations and no prose. T10.2
is T4.10 and T9.12 a
...
T10.3's own resolution needed a second half before this task could close. The gate was
first registered in `docs.yml` alone, whose `paths:` cover content and not `crates/**`, so
the 34 tests would have run when the TESTS moved and never when the binary that runs them
moved — the same omission the entry filed, pointed the other way. It is now in both
workflows, which is where both doc scripts already were, for the reason `test.yml`'s
header had already written down.

T10.4 was not on the assignment and is the entry this task most nearly lost. The scaffold
README overwritten in Step 2 had named a scene deleted in Task 1 for nine tasks; replacing
it without logging it would have deleted the measurement, which is the protocol's own rule
broken by the task whose deliverable is the log. It is T1.9's class — a scaffolded
artifact rotting in silence — on a new artifact class, and its verdict splits from T1.9's
`ERGONOMIC` precisely because prose is the one artifact no checker can ever be made
responsible for: the only place the remedy can live is in what `lute init` writes.
