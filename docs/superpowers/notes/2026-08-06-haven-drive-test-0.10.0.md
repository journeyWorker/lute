...
`docs/examples/haven/` is not a sample. It is an **instrument**. The eleven-scene example
was built in July to drive-test the language; that run produced
[`2026-07-31-haven-drive-test-findings.md`](2026-07-31-haven-drive-test-findings.md) —
...
> Haven corpus costs the entire scene graph, six compiles and all 31 tests).
...
```
$ ./target/debug/lute check-project docs/examples/haven
ok: docs/examples/haven (18 file(s), 13 project-wide warning(s))
rc=0
...
`goto` text hit is prose at `haven/scenes/spine-b.lute:101`, inside the block comment opened
...
`docs/examples/haven/world.schema.yaml:17`, not `:16` — line 16 is the `facts:` key itself.
...
plugin-less scratch document under each root: `haven`/`episodes`/`investigation` at
`0678492f…` (identical to the outer root), `showcase` `2a5eaaeb…`,
`plugindef-project` `1235ef58…`, `arcia-project` `c7fa83a9…`. **The counter-example the
spec warns about also reproduced:** measuring a real document under `arcia-project/` gives
...
`docs/examples/haven/README.md:97` now reads *"Two things in this tree are wrong on
...
**The acceptance test passes.** `docs/examples/haven/scenes/spine-a.lute:37` reads
...
construction. Its only two declared exits (`haven/scenes/wake.lute:14`,
`marina-s01ep02.lute:190`) are each the last staging event for their character — verified by
...
`/tmp` copy of `haven/vocabulary.schema.yaml` takes the project from 13 to 14 warnings with
...
/tmp/ProbeT12/ex/haven/lute.project.yaml: warning [W-PROJECT-INERT] this manifest does not
govern under `--project /tmp/ProbeT12/ex` and would have resolved different `identity:`
templates; its settings are not applied to any document. Invoke this root directly to use
them (0.10.0 §7)
… (5 more: episodes, arcia-project, investigation, plugindef-project, showcase)
```

Every Haven artifact built from the outer root silently changes its `lineId`s and
...
```
$ lute trace docs/examples/haven/components/purser-interject.component.lute
...
$ cd docs/examples/haven && lute check --project . components/purser-interject.component.lute
...
author-facing diagnostic. §9 delivered the caller-less half; Haven's component **is**
...
**The backlog's own Verify, run in full.** A copy of the Haven corpus with `defaults:`
...
$ lute check-project /tmp/Complement/havenD | tail -1
failed: /tmp/Complement/havenD (18 file(s), 16 project-wide error(s), 2 project-wide warning(s))
$ lute scenario /tmp/Complement/havenD | head -4
project root: /tmp/Complement/havenD
  topological layers:
...
$ lute test /tmp/Complement/havenD | tail -1
0 passed, 31 failed
```
...
1. **`loc export` is now expansion-scoped, so an unreachable `<match>` arm leaves the
   localization bundle with no warning.** Haven's component `<otherwise>` line
   *"Allocation is nominal."* appears in **zero** export rows, because the corpus's single
   ...
   interacts with the deliberate imperfection `docs/examples/haven/README.md` item 1
   documents. `ERGONOMIC`.
...
3. **`docs/examples/haven/scenes/opening.lute`** does not exist — Haven is now eleven named
...
4. **T2.4's project-level run needed `episode: 99`** rather than the log's `episode: 2`,
   because the corpus grew and `cryobank.lute` now owns `haven.s01ep02`
