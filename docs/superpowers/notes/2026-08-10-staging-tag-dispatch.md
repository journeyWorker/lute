...
Two defects landed in 0.10.1 out of a real adoption project — an Stage visual-novel
...
```
# docA/a.lute frontmatter: kind: scene, character: eve, season: 1, episode: 1, uses: schema.yaml
## Room A
::bg{location="room_a"}
::auto{character="eve" action="walk-in"}
@eve{code="0010"}: Hello from room A.
::bg{location="room_b"}
@narrator{code="0020"}: Eve is never shown again.
```

```
# docB/b.lute frontmatter: kind: scene, character: eve, season: 1, episode: 1,
...
@@ -2,6 +2,5 @@
 {"kind":"sprite","addr":"001-0200","character":"eve","action":"walk-in"}
 {"kind":"sprite","addr":"001-0300","character":"eve","anchor":"center","provenance":{"injected":true,"by":"auto-anchor-on-show","explanation":"`eve` shown without an explicit anchor; defaulting to `center`"}}
 {"kind":"line","addr":"001-0400","role":"dialogue","speaker":"eve","text":"Hello from room A.","lineId":"eve.s01ep01.eve_0010","voiceKey":"eve-0010"}
-{"kind":"sprite","addr":"001-0500","character":"eve","exit":true,"provenance":{"injected":true,"by":"stage-bookkeeping","explanation":"auto-hiding `eve` left on stage across a scene change"}}
-{"kind":"background","addr":"001-0600","location":"room_b","wait":true}
-{"kind":"line","addr":"001-0700","role":"narration","speaker":"narrator","text":"Eve is never shown again.","lineId":"eve.s01ep01.narrator_0020"}
+{"kind":"background","addr":"001-0500","location":"room_b","wait":true}
+{"kind":"line","addr":"001-0600","role":"narration","speaker":"narrator","text":"Eve is never shown again.","lineId":"eve.s01ep01.narrator_0020"}
```
...
matches `d.tag == "bg"` on `inject.rs:182`, so `stage_bookkeeping_bg` never runs, `eve`'s
...
adjacent gap below shows is a live possibility, not a strawman) leaves `eve` visible in
...
So the naive test — add a second line for `eve` after the scene change in `docA`/`docB`
...
directly — `eve` explicitly exits in scene 1, the scene changes, `eve` speaks again in scene
2 with no re-show:
...
/tmp/stage-dispatch/docB3/b3.lute:23:1: warning [W-STAGE-ABSENT] `eve` left the stage on an
...
/tmp/stage-dispatch/docPC/pc.lute:17:1: warning [W-STAGE-ABSENT] `eve` left the stage on an
...
(`inject.rs:233-247`) instead, which runs neither rule. Two full scenes, `eve` walks in at
the top of each:

```
$ lute compile … docA4   # core ::bg — SECOND ::auto correctly treated as an entrance
… scene 2 …
{"kind":"sprite","addr":"002-0300","character":"eve","anchor":"center","provenance":{"injected":true,"by":"auto-anchor-on-show", …}}
{"kind":"sprite","addr":"002-0500","character":"eve","preload":true,"emotion":"surprised","provenance":{"injected":true,"by":"entry-emotion-lookahead", …}}
...
… scene 2 …
{"kind":"sprite","addr":"002-0200","character":"eve","action":"walk-in"}
...
`eve` speaking `surprised` with no sprite pre-loaded at that emotion, and no anchor set for
...
`::place` clears neither. Scene 1: `eve` speaks a stateful (`emotion=`) line, marking her
dirty. Scene changes. Scene 2: `eve` speaks a **plain** line, no re-`::auto` at all:
...
{"kind":"sprite","addr":"002-0200","character":"eve","posReset":true,"provenance":{"injected":true,"by":"auto-pose-reset","explanation":"`eve` had a dirty pose before a plain line; resetting to neutral"}}
{"kind":"line","addr":"002-0300", …}
```

`docB5` injects a `posReset` sprite command **targeting a character the artifact never
placed in that scene** — the reducer believes `eve` is still on stage from scene 1 (nothing
...
`TOOL-DEFECT` in `2026-08-06-haven-drive-test-0.10.0.md`). The fix is well-understood and
...
2. **`::auto{character="eve" action="enter"}`** is not a legal construction against any
   schema shipped in this repo — no `action` domain member in `docs/examples/base.schema.yaml`
   or `docs/examples/haven/vocabulary.schema.yaml` is named `enter`. I substituted a real
...
4. **The field evidence's "surfaced three `W-STAGE-ABSENT` reports"** is reported to me, not
   independently re-derived — I do not have the Stage adoption project's repository in this
