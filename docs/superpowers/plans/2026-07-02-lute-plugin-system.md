...
**Goal:** Load capability plugins from disk (`plugins/<id>/`), resolve a profile into ONE multi-plugin capability snapshot the checker/LSP/CLI all consume, so `docs/examples/date-minigame.lute` — with the `arcia.minigame` plugin present — validates **clean** (`ok:true`); and first fix four real checker `defassign`×`exhaustiveness` bugs (Group D) that the plugin work would otherwise build on top of.
...
- Reference example: `docs/examples/date-minigame.lute` + Appendix A (`arcia.minigame`).
...
- `docs/examples/arcia-project/` — **NEW.** on-disk reference project: `lute.project.yaml`, `plugins/arcia.minigame/` (Appendix A verbatim), `catalog/minigame.yaml` (provider snapshot with `marina_service_01`).
...
```bash
./target/debug/lute check docs/examples/marina-s01ep02.lute --json   # ok:true, exit 0
...
```rust
    ...
    fn map_option_values_deep_merge_across_layers() {
        ...
        use std::collections::BTreeMap;
        // parent sets cast.marina={costume:a}; child adds cast.ren={costume:b}.
        let mut parent_opt = BTreeMap::new();
        let mut cast_p = BTreeMap::new();
        cast_p.insert("marina".to_string(), Literal::Str("a".into()));
        ...
        match plug.options.get("cast").unwrap() {
            Literal::Map(m) => {
                assert!(m.contains_key("marina"), "parent entry retained");
...
```rust
    ...
    fn installed_plugins_lookup() {
        ...
        let reg = InstalledPlugins {
            by_id: BTreeMap::from([("arcia.minigame".to_string(), InstalledPlugin { manifest: m })]),
        };
        assert_eq!(reg.get("arcia.minigame").unwrap().manifest.version, "0.1.0");
...
```rust
    ...
    fn dependency_closure_pulls_transitive_deps() {
        use std::collections::BTreeMap;
        // story activates arcia.vn; arcia.vn depends arcia.base; base depends lute.core.
        let graph = ProfileGraph {
            profiles: BTreeMap::from([(
                "story".to_string(),
                Profile { extends: None, plugins: BTreeMap::from([("arcia.vn".to_string(), BTreeMap::new())]) },
            )]),
            default_profile: "story".to_string(),
        };
        let inst = installed(vec![
            manifest("lute.core", "0.0.1", &[]),
            manifest("arcia.base", "0.1.0", &[("lute.core", "^0.0.1")]),
            manifest("arcia.vn", "0.1.0", &[("arcia.base", "^0.1.0")]),
        ]);
        let active = resolve_activation(&graph, "story", &BTreeMap::new(), &inst).unwrap();
        let ids: Vec<_> = active.iter().map(|a| a.id.as_str()).collect();
        assert!(ids.contains(&"arcia.base"), "transitive dep must be activated: {ids:?}");
        assert!(ids.contains(&"arcia.vn"));
...
```rust
    ...
    fn inactive_does_not_change_the_version() {
        ...
        a.inactive.insert("minigame".into(), "arcia.minigame".into());
...
fn active_plugin_directive_lands_in_snapshot() {
    let reg = InstalledPlugins {
        by_id: BTreeMap::from([(
            ...
            InstalledPlugin { loaded: plugin_with_directive("arcia.minigame", "minigame") },
        )]),
    };
    let active = vec![
        ActivePlugin { id: "lute.core".into(), options: BTreeMap::new() },
        ActivePlugin { id: "arcia.minigame".into(), options: BTreeMap::new() },
    ];
...
fn inactive_plugin_is_indexed_not_merged() {
    ...
    assert_eq!(snap.inactive.get("minigame"), Some(&"arcia.minigame".to_string()));
}
```
...
<when test=\"$ == 'gold'\">:line[marina]: a\n</when>\n\
<otherwise>:line[marina]: b\n</otherwise>\n\
</match>\n";
...
fn unknown_tag_from_inactive_plugin_gets_fixit() {
    let mut snap = lute_manifest::core::load_core_snapshot();
    snap.inactive.insert("minigame".into(), "arcia.minigame".into());
    ...
    assert!(d.fixits.iter().any(|f| f.title.contains("arcia.minigame")), "fix-it names the plugin: {:?}", d.fixits);
}
```
...
profiles:
  ...
  story:
    plugins: { arcia.minigame: true }   # etc.
  date-minigame:
    extends: story
    plugins:
      arcia.minigame: { resultScope: scene, allowedKinds: [rhythm] }
```
...
fn write_project(root: &std::path::Path) {
    ...
    fs::write(root.join("plugins/arcia.minigame/plugin.yaml"),
        "id: arcia.minigame\nversion: 0.1.0\nkind: capability\nexports:\n  directives: directives/\n").unwrap();
...
### Task 7.2: on-disk reference project fixture (`arcia.minigame`)

**Files:**
- Create: `docs/examples/arcia-project/lute.project.yaml`
- Create: `docs/examples/arcia-project/plugins/arcia.minigame/plugin.yaml` (Appendix A)
- Create: `docs/examples/arcia-project/plugins/arcia.minigame/state/shapes.yaml` (Appendix A)
- Create: `docs/examples/arcia-project/plugins/arcia.minigame/directives/minigame.yaml` (Appendix A)
- Create: `docs/examples/arcia-project/plugins/arcia.minigame/providers/minigame.yaml`
- Create: `docs/examples/arcia-project/plugins/arcia.minigame/bridge/minigame.yaml`
- Create: `docs/examples/arcia-project/catalog/minigame.yaml` (provider snapshot with `marina_service_01`)

**Interfaces:** none (data fixture). This is the reference project the acceptance tests point at. `date-minigame.lute` selects `profile: date-minigame` and scene-local `plugins: { arcia.minigame: { resultScope: scene, allowedKinds: [rhythm] } }` — the project must define that profile.

- [ ] **Step 1: Write the manifest files verbatim from Appendix A**

`plugins/arcia.minigame/plugin.yaml`:
```yaml
id: arcia.minigame
version: 0.1.0
kind: capability
...
`plugins/arcia.minigame/state/shapes.yaml`:
```yaml
stateShapes:
  - name: minigameResult
    fields:
...
`plugins/arcia.minigame/directives/minigame.yaml`:
```yaml
directives:
  - name: minigame
    layer: bridge
...
`plugins/arcia.minigame/providers/minigame.yaml`:
```yaml
providers:
  - name: minigameId
    idShape: "Ident"
...
`plugins/arcia.minigame/bridge/minigame.yaml`:
```yaml
bridgeCapabilities:
  - service: minigame
    operation: play
...
Run a quick check that `load_plugins_dir` reads the package cleanly (add a throwaway assertion in `project.rs` tests pointing at `../../docs/examples/arcia-project`, or verify in Task 7.4's acceptance). No standalone commit test needed — this is data.

- [ ] **Step 3: Commit the fixture**

```bash
git add docs/examples/arcia-project
git commit -m "test(fixture): on-disk arcia.minigame reference project (plugin Appendix A)"
```
...
fn date_minigame_is_clean_with_plugin_project() {
    let out = Command::new(lute_bin())
        .args([
            ...
            "--project", "../../docs/examples/arcia-project",
            "--providers", "../../docs/examples/arcia-project/catalog",
            "--json",
        ])
        .output()
        .expect("run lute");
...
fn divergence_holds_under_plugin_project() {
    ...
    let proj = load_project(std::path::Path::new("../../docs/examples/arcia-project")).unwrap();
    ...
    let providers = lute_manifest::provider::ProviderSet::load("../../docs/examples/arcia-project/catalog");
...
- [ ] **Step 2: Run to verify failure** — `cargo test -p lute-lsp --test divergence divergence_holds_under_plugin` → may FAIL first if `date-minigame` isn't clean because the `allowedKinds` enum (`enumFromOption`) or providerRef isn't satisfied. Diagnose: the scene uses `kind="rhythm"` (in `allowedKinds`) and `id="marina_service_01"` (in the catalog) — both must resolve. If `E-BAD-ENUM`/`E-UNKNOWN-ID` appears, verify (a) the resolved `ResolvedPlugin.options` carries `allowedKinds` so `resolve_option_domain` finds it, and (b) `--providers` catalog has `minigameId: [marina_service_01]`. Fix data/wiring until clean.
...
fn refresh_stamps_resolved_version_under_project() {
    ...
    std::fs::copy("../../docs/examples/arcia-project/catalog/minigame.yaml", tmp.join("minigame.yaml")).unwrap();
    let out = std::process::Command::new(lute_bin())
        .args(["catalog", "refresh", tmp.to_str().unwrap(), "--project", "../../docs/examples/arcia-project"])
...
./target/debug/lute check docs/examples/date-minigame.lute --json \
    --project docs/examples/arcia-project --providers docs/examples/arcia-project/catalog   # ok:true, exit 0
```
