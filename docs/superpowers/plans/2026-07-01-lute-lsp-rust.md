...
- **Scope out:** final `arcia_script_commands` flat-record codegen; runtime CEL evaluation; the warm daemon. Our "lowering" stops at the LSP-facing resolved view.
...
mod tests {
    ...
    fn graph() -> ProfileGraph {
        ...
        profiles.insert("story".into(), Profile { extends: None, plugins: map(&[("arcia.vn", opts(&[]))]) });
        profiles.insert("date".into(), Profile { extends: Some("story".into()), plugins: map(&[("arcia.date", opts(&[]))]) });
        profiles.insert("date-minigame".into(), Profile {
            extends: Some("date".into()),
            plugins: map(&[("arcia.minigame", opts(&[("resultScope", Literal::Str("scene".into()))]))]),
        });
    ...
    fn resolves_extends_chain_parent_first_with_core_and_global() {
        ...
        assert_eq!(ids, vec!["lute.core", "arcia.vn", "arcia.date", "arcia.minigame"]);
    }

    #[test]
    fn scalar_option_later_layer_overrides() {
        let g = graph();
        let scene_local = map(&[("arcia.minigame", opts(&[("resultScope", Literal::Str("run".into()))]))]);
        let active = resolve_activation(&g, "date-minigame", &scene_local).unwrap();
        let mg = active.iter().find(|a| a.id == "arcia.minigame").unwrap();
...
mod tests {
    ...
    fn version_is_deterministic_and_order_independent() {
        let a = CapabilitySnapshot::builder()
            ...
            .plugin("arcia.minigame", "0.1.0", &[])
            .build();
        let b = CapabilitySnapshot::builder()
            .plugin("arcia.minigame", "0.1.0", &[])   // reversed insert order
            .plugin("lute.core", "0.0.1", &[])
            .build();
...
mod tests {
    ...
    fn absent_id_in_fresh_snapshot_is_absent() {
        let ps = ProviderSnapshot {
            ...
            entries: [("character".to_string(), vec!["marina".to_string()])].into(),
            stale: false,
        };
        let set = ProviderSet::from_one(ps);
        assert_eq!(set.contains("character", "marina"), IdStatus::Fresh);
...
mod tests {
    ...
    fn peels_yaml_frontmatter() {
        ...
        assert!(fm.unwrap().0.contains("character: marina"));
...
```rust
#[test]
fn parses_marina_example_without_parse_errors() {
    let text = std::fs::read_to_string("../../docs/examples/marina-s01ep02.lute").unwrap();
...
Expected: PASS including the marina example integration test.
...
mod tests {
    ...
    fn parses_valid_cel_and_records_ast() {
        ...
        let h = parse_slot(&mut arena, "scene.affect.marina >= 1", 0).unwrap();
...
mod tests {
    ...
    fn number_increment_ok() {
        let errs = check_set(&set("scene.affect.marina", "+=", "1"), &schema_number(), &ctx());
        assert!(errs.is_empty(), "{errs:?}");
    }
}
```
...
mod tests {
    ...
    fn show_without_anchor_injects_anchor_with_provenance() {
        ...
        let (st2, injected) = lower_node(st, &show_marina_no_anchor(), &[]);
    ...
    fn dirty_pose_before_nonstateful_line_injects_posreset() {
        ...
        st.on_stage.insert("marina".into(), SpriteState::default());
        let (_st2, injected) = lower_node(st, &line_marina(), &[]);
...
fn check_clean_file_exits_zero_json() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_lute"))
        .args(["check", "../../docs/examples/marina-s01ep02.lute", "--json"])
        .output().unwrap();
...
fn hover_on_ref_shows_def_cel() {
    ...
    assert!(h.contents_text().contains("scene.affect.marina >= 1"));
}
#[test]
fn definition_on_choices_path_jumps_to_branch() {
    let loc = definition_at(&parsed_marina(), &snap(), pos_on("scene.choices.number")).unwrap();
...
- [ ] **Step 1–5:** Test-first per feature (assert fold count on the marina example = shots + timeline + branch + match; assert a `::camera` token carries the staging layer; assert 5 shot symbols). Implement, wire, commit.
...
- **Final `arcia_script_commands` codegen** and **runtime CEL evaluation** — engine-owned (see Global Constraints).
...
- [ ] `lute check docs/examples/marina-s01ep02.lute --json` and `... date-minigame.lute --json` — both `ok: true`.
...
- **Scope discipline:** stop at the resolved view. Do not emit `arcia_script_commands`.
