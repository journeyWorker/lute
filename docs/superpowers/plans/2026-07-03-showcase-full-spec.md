...
- Every showcase scene MUST `lute check` exit 0 (no masking). Study existing fixtures for correct syntax: `docs/examples/{marina-s01ep02.lute, carry-ep.lute, state.schema.lute, extends-*.lute, choice-persist.lute, property-tracks.lute, param-def.lute, plugin-def.lute, arcia-project/*}`.
...
- `plugins/showcase.pack/directives/*.yaml` — ≥1 bridge/staging directive (with attrs: a `providerRef` and an `assetKind` typed attr), following `arcia-project/plugins/arcia.minigame/directives/minigame.yaml`.
...
- `plugins/showcase.pack/assetkinds/*.yaml` — an asset kind (segments), following `arcia.minigame/assetkinds/art.yaml`.
- `plugins/showcase.pack/defs/*.yaml` — a plugin-exported def (list form `- { name, type, cel }`).
- `catalog/*.yaml` — the provider catalog (if the project resolves providers from `catalog/`, per arcia-project) so providerRef ids resolve.
