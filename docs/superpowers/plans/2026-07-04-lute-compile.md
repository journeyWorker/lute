...
crates/lute-compile/
  ...
  tests/e2e.rs          (Task 14) marina/showcase/components full-artifact insta goldens
crates/lute-check/
...
fn injected_sprite_carries_provenance() {
    let cmd = Command::Sprite(SpriteCmd {
        ...
        stamp: Stamp {
            provenance: Some(lute_check::Provenance {
                ...
                reason: "pre-loading marina's first emotion".into(),
            }),
            ..Stamp::default()
        },
    });
    assert_eq!(
        j(&cmd),
        r#"{"kind":"sprite","addr":"002-0200","character":"marina","preload":true,"emotion":"surprised","provenance":{"injected":true,"by":"entry-emotion-lookahead","reason":"pre-loading marina's first emotion"}}"#
    );
}
...
fn match_jump_barrier_serialize() {
    let m = Command::Match(MatchCmd {
        ...
        arms: vec![MatchArm { test: "(scene.affect.marina >= 1)".into(), target: "005-0800".into() }],
    ...
    assert_eq!(
        j(&m),
        r#"{"kind":"match","addr":"005-0700","subject":"scene.choices.number","arms":[{"test":"(scene.affect.marina >= 1)","target":"005-0800"}],"otherwise":"005-1200","converge":"005-1400"}"#
    );
...
fn stamped_camera_and_set_and_plugin_passthrough() {
    ...
    assert_eq!(
        j(&set),
        r#"{"kind":"set","addr":"004-0900","path":"scene.affect.marina","op":"+=","value":"1"}"#
    );
...
fn envelope_serializes_with_state_entries() {
    let a = Artifact {
        lute: "0.0.1".into(),
        meta: ArtifactMeta { character: "marina".into(), season: 1, episode: 2, episode_id: "S01EP02".into(), title: Some("T".into()) },
    ...
    assert_eq!(
        ...
        r#"{"lute":"0.0.1","meta":{"character":"marina","season":1,"episode":2,"episodeId":"S01EP02","title":"T"},"state":[{"path":"scene.choices.number","type":"enum","domain":["blunt","soft","unset"],"provenance":"branch:number"}],"commands":[]}"#
    );
}
```
...
fn fold_env_exposes_folded_schema_and_def_bodies() {
    ...
    // Inline decl folded.
    assert!(folded.env.state.decls.contains_key("scene.affect.marina"));
...
mod tests {
    ...
    fn bare_ref_expands_parenthesized() {
        let t = tables(&[("fond", "scene.affect.marina >= 1")], &[]);
        assert_eq!(expand("@fond", &t, None).unwrap(), "(scene.affect.marina >= 1)");
    }

    #[test]
    fn fn_ref_binds_args_positionally_and_parenthesized() {
        let t = tables(&[("atLeast", "scene.affect.marina >= n")], &[("atLeast", &["n"])]);
        assert_eq!(
...
mod tests {
    ...
    fn use_expands_component_inline_with_bound_params_and_sentinels() {
        ...
        assert!(matches!(&ch.value, AttrValue::Str(s) if s == "marina"), "{ch:?}");
...
mod tests {
    ...
    fn line_roles_derive_from_speaker_and_delivery() {
        ...
        let v = lower_first(
            ":line[marina]{code=\"0010\" emotion=\"surprised\" variant=\"0\" as=\"Hostess\"}: Oh!",
        );
        assert_eq!(v["role"], "dialogue");
        assert_eq!(v["speaker"], "marina");
        assert_eq!(v["text"], "Oh!");
    ...
    fn set_ops_lower_verbatim() {
        for op in ["=", "+=", "-=", "*="] {
            let v = lower_first(&format!("::set{{scene.affect.marina {op} 1}}"));
    ...
    fn use_and_sentinels_lower_to_nothing() {
        let ns = nodes("::use{component=\"greet\" who=\"marina\"}");
...
const BRANCH: &str = r#"<branch id="number">
  <choice id="blunt" label="Just ask, flatly">
    :line[fixer]{code="0050"}: Marina. Your number.
  </choice>
...
fn anchor_and_preload_inject_after_authored_auto() {
    ...
    let (recs, _) = walk(
        "::auto{character=\"marina\" action=\"fade-in-up\"}\n:line[marina]{emotion=\"surprised\"}: Oh!",
    );
    let sprites: Vec<String> = recs.iter().filter_map(|r| sprite_desc(&r.cmd)).collect();
    assert_eq!(
        sprites,
        vec![
            "marina:show:authored".to_string(),
            "marina:anchor:auto-anchor-on-show".to_string(),
            "marina:preload:entry-emotion-lookahead".to_string(),
        ]
    );
...
:line[marina]{emotion=\"delighted\" action=\"pose-lean\"}: A!\n\
:line[marina]: B.";
...
fn branch_arms_fork_from_entry_state_and_join_conservatively() {
  ...
  <choice id="b" label="B">
    ::auto{character="marina" action="fade-in-up"}
    :line[marina]{emotion="delighted"}: Ha!
  </choice>
</branch>
::auto{character="marina" action="fade-in-up"}"#;
    let (recs, _) = walk(body);
...
fn join_states_unit_semantics() {
    ...
    a.on_stage.insert("marina".into(), sprite("surprised"));
    a.on_stage.insert("kenshi".into(), sprite("neutral"));
    a.dirty.insert("kenshi".into());
    a.bg = Some("street".into());
    let mut b = StageState::default();
    b.on_stage.insert("marina".into(), sprite("delighted"));
    b.on_stage.insert("kenshi".into(), sprite("neutral"));
    ...
    assert!(joined.dirty.contains("kenshi"));
...
fn addressed(src: &str) -> (Vec<Command>, Vec<lute_core_span::Diagnostic>) {
    ...
    assign_addresses(shots, &IdCx { character: "marina", season: 1, episode: 2 })
}
...
<branch id="number">
  <choice id="blunt" label="Just ask, flatly">
    :line[marina]{code="0010" emotion="surprised"}: Oh!
  </choice>
  <choice id="soft" label="Ask gently">
    ::set{scene.affect.marina += 1}
  </choice>
...
fn line_ids_and_voice_keys_follow_the_speaker_code_model() {
    ...
    assert_eq!(
        ...
        vec![
            // Authored code kept.
            ("fixer", "marina.s01ep02.fixer_0010", Some("fixer-0010")),
            // Narrator: lineId for i18n, NO voiceKey (unvoiced role).
            ("narrator", "marina.s01ep02.narrator_0010", None),
            ("fixer", "marina.s01ep02.fixer_0050", Some("fixer-0050")),
            // Back-filled: fixer's max authored code is 0050 => next is 0060.
            ("fixer", "marina.s01ep02.fixer_0060", Some("fixer-0060")),
            ("marina", "marina.s01ep02.marina_0010", Some("marina-0010")),
        ]
    );
    ...
    assert_eq!(choice.options[0].line_id, "marina.s01ep02.number.blunt");
    assert_eq!(choice.options[1].line_id, "marina.s01ep02.number.soft");
}
```
...
fn clean_doc_compiles_with_envelope_expansion_and_ids() {
    ...
    assert_eq!(artifact.meta.character, "marina");
    ...
    assert_eq!(paths, vec!["scene.affect.marina", "scene.choices.number"]);
...
#[test]
fn compile_marina_exits_zero_with_artifact_json() {
    let out = Command::new(BIN)
        .args(["compile", "../../docs/examples/marina-s01ep02.lute"])
        .output()
        .unwrap();
    assert_eq!(
        out.status.code(),
...
fn compile_writes_out_file() {
    ...
    let out = Command::new(BIN)
        .args(["compile", "../../docs/examples/marina-s01ep02.lute", "-o"])
        .arg(&tmp)
        .output()
        .unwrap();
...
Expected: FAIL — exit code mismatch / clap error `unrecognized subcommand 'compile'` (surfaces as code 2 from clap; the marina test asserts 0 and fails).
...
Full-artifact `insta` snapshots for the three real fixtures (§8): `marina-s01ep02.lute` (core), `showcase/episode01.lute` (project + plugins + imports + components), `components/scene.lute` (component expansion) — plus a structural invariant checker and the byte-determinism assertion.

**Files:**
- Test: `crates/lute-compile/tests/e2e.rs`
- Create (generated, then accepted): `crates/lute-compile/tests/snapshots/e2e__marina_s01ep02.snap`, `…__showcase_episode01.snap`, `…__components_scene.snap`
...
#[test]
fn marina_s01ep02() {
    golden("marina_s01ep02", "../../docs/examples/marina-s01ep02.lute", None);
}
...
- `marina_s01ep02`: shot-2 `::auto` sprite followed by an injected `preload` sprite with `entry-emotion-lookahead` provenance (§4.5); the shot-3 timeline records stamped `"timeline": 1` with a `barrier` at `1.4`; the shot-4 `choice` with `recordKey: "scene.choices.number"` and resolved targets; shot-5 `match` with the `@fond` arm expanded to `(scene.affect.marina >= 1)`.
...
- `components_scene`: the greet body inline with `character: "marina"` bound and `source: { component: "greet" }`.
...
git commit -m "test(compile): e2e artifact goldens + determinism (marina/showcase/components)"
```
...
$target_bin compile docs/examples/marina-s01ep02.lute -o /tmp/lute-a.json
$target_bin compile docs/examples/marina-s01ep02.lute -o /tmp/lute-b.json
