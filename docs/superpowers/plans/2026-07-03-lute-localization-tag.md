...
- [ ] **Step 5: full gate**
  ...
  - Acceptances unchanged: marina 0 · date-minigame core-only 1 · `--project` 0 · arcia-portrait `--project` 0 · carry-ep 0. NEW: `lute tag` on a temp copy of marina is a no-op (marina is fully coded) — `./target/debug/lute tag <temp-copy-of-marina>` prints "already tagged"/adds 0 and leaves the file byte-identical (ad-hoc check).
...
- [ ] Acceptances: marina 0 / dm-core 1 / dm-proj 0 / portrait 0 / carry-ep 0; `lute tag` on a fully-coded doc is a no-op; on an untagged doc back-fills `code`s and is idempotent.
