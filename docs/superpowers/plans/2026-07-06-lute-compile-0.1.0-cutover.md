...
- [ ] **Step 1:** failing test — a line `:marina: Hi {{userName}}, {{run.coins}} left, {{@fond}}.` → LineCmd `placeholders: [{kind:reserved,ref:userName},{kind:path,ref:run.coins},{kind:ref,ref:@fond}]`, `text` unchanged.
- [ ] **Step 2:** run → FAIL.
