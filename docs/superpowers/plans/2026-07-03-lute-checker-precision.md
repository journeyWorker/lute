...
fn write_targets_set_clip_is_its_path() {
    ...
    let node = set_clip("scene.affect.marina"); // ClipNode::Set { path: "scene.affect.marina", .. }
    assert_eq!(
        clip_write_targets(&node, &snap, Some("marina")),
        WriteTargets::Paths(["scene.affect.marina".to_string()].into_iter().collect())
    );
}
...
- [ ] Acceptances unchanged: `./target/debug/lute check docs/examples/marina-s01ep02.lute` exit 0; `date-minigame.lute` core-only exit 1; `--project docs/examples/arcia-project` exit 0; `arcia-portrait.lute --project` exit 0.
