...
- **Enforcement posture = A-hybrid (locked):** the graph is emitted to IR as *advisory* data only (like `relations:`/`rules:`). No normative engine gate. Every §4.2/§4.3/§4.4 diagnostic message MUST carry the verbatim qualifier **"under your declared routes"** (spec §2.6) — asserted by a lint-level test.
...
- **Determinism:** all project-wide collections keyed on `BTreeMap`/`BTreeSet` (byte-stable output, IR byte-stability contract). IR `Artifact` field declaration order IS the serialized order (`ir.rs:1-3`, `ir.rs:15-50`); the new advisory field is appended **after `commands`** — the current LAST field (`ir.rs:48`) — a true append-only change, `commands` and all prior fields unmoved. `#[serde(skip_serializing_if = "Vec::is_empty")]`.
...
fn corpus_no_false_positive_episode_dup_across_subprojects() {
    // run check-project over docs/examples/; the demo/marina cross-subproject reuse must NOT flag E-CONN-EPISODE-ID-DUP
