...
**Goal: measure whether Lute 0.9.0 is mature enough to author a real work in.** The eleven-scene, six-quest Haven prologue is the *instrument*, not the deliverable. The deliverable is `docs/superpowers/notes/2026-07-31-haven-drive-test-findings.md` — a running log of everything the language could not express, expressed badly, or expressed only after reading its own source.
...
**Architecture:** A project root at `docs/examples/haven/`, scaffolded by `lute init`, then filled in. Scenes are ordered by `after:` routes; quests gate on relational queries; scenario tests pin the endings.
...
| Identity tokens are exactly `{prefix}`, `{speaker}`, `{code}`; `prefix` = `{character}.s{season}ep{episode}` | compiled `lineId` = `haven.s01ep01.vesna_0010` |
...
`docs/superpowers/notes/2026-07-31-haven-drive-test-findings.md` before starting and append to it
...
- Branch `example/haven-prologue` is checked out. Commit there.
...
**Files:**
- Create (via `lute init`): `docs/examples/haven/{lute.project.yaml,vocabulary.schema.yaml,world.schema.yaml,scenes/opening.lute,mocks/playthrough.yaml,README.md}`
...
```bash
./target/debug/lute init docs/examples/haven
```
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, 1 file
./target/debug/lute compile docs/examples/haven/scenes/wake.lute -o /tmp/t1.json
grep -o '"lineId": *"[^"]*"' /tmp/t1.json | head -2
```
Expected: `haven.s01ep01.vesna_0010`, `haven.s01ep01.vesna_0020`
...
```bash
./target/debug/lute compile docs/examples/haven/scenes/wake.lute -o /tmp/t2.json
python3 -c "
...
- Produces: `haven.s01ep02`, the route ancestor of everything downstream.
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, 2 files
```
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, 3 files
```
...
**Files:**
- Create: `docs/examples/haven/scenes/bridge.lute` (episode 10)
- Create: `docs/examples/haven/scenes/shed.lute` (episode 11)
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, 5 files
```
Add a content line after `::end` in `shed.lute`, re-check, confirm `W-CODE-AFTER-END`, remove it.

```bash
./target/debug/lute compile docs/examples/haven/scenes/bridge.lute -o /tmp/t5.json
...
- Modify: `docs/examples/haven/scenes/cryobank.lute`
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, 6 files
```
...
- [ ] **Step 2: Verify** — `./target/debug/lute check-project docs/examples/haven` → ok, 10 files
...
`bridge.lute` → `after: 'visited("haven.s01ep09")'`; `shed.lute` → `after: 'visited("haven.s01ep07")'`.

- [ ] **Step 3: Verify the graph**

```bash
./target/debug/lute check-project docs/examples/haven   # ok, 13 files
./target/debug/lute scenario docs/examples/haven
```
...
```bash
./target/debug/lute check-project docs/examples/haven   # ok, all files
./target/debug/lute test docs/examples/haven
```
...
**Files:**
- Modify: `docs/examples/haven/README.md` (replace the `lute init` stub)
...
```
Both print the roots they cover. If `docs/examples/haven` is not covered by the existing `docs/examples` root, register it. Confirm; do not assume.
