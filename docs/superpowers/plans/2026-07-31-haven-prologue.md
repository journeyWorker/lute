# Haven Prologue Implementation Plan
...
- Branch `example/haven-prologue` is already checked out. Commit there.
...
**Files:**
- Create: `docs/examples/haven/lute.project.yaml`
- Create: `docs/examples/haven/vocabulary.schema.yaml`
- Create: `docs/examples/haven/world.schema.yaml`
- Create: `docs/examples/haven/scenes/wake.lute`
...
Run: `./target/debug/lute check-project docs/examples/haven`
Expected: `ok: docs/examples/haven (1 file(s), 0 project-wide warning(s))`

- [ ] **Step 6: Verify the identity template took effect**

Run: `./target/debug/lute compile docs/examples/haven/scenes/wake.lute -o /tmp/haven-t1.json && grep -o '"lineId": *"[^"]*"' /tmp/haven-t1.json | head -2`
Expected: `haven.s01ep01.vesna_0010` and `haven.s01ep01.vesna_0020`
...
```bash
git add docs/examples/haven
git commit -m "feat(example): Haven project root, vocabulary, and world schema"
```
...
**Files:**
- Modify: `docs/examples/haven/scenes/wake.lute`
...
Run: `./target/debug/lute compile docs/examples/haven/scenes/wake.lute -o /tmp/haven-t2.json && grep -o '"exit": *true' /tmp/haven-t2.json | wc -l`
Expected: `1`
...
```bash
git add docs/examples/haven/scenes/wake.lute
...
**Files:**
- Create: `docs/examples/haven/scenes/cryobank.lute`
...
Run: `./target/debug/lute compile docs/examples/haven/scenes/cryobank.lute -o /tmp/haven-t3.json && grep -c 'shedPressure' /tmp/haven-t3.json`
...
```bash
git add docs/examples/haven/scenes/cryobank.lute
...
**Files:**
- Create: `docs/examples/haven/quests/hold-the-spine.lute`
...
```bash
git add docs/examples/haven/quests/hold-the-spine.lute
...
**Files:**
- Create: `docs/examples/haven/scenes/bridge.lute`
- Create: `docs/examples/haven/scenes/shed.lute`
...
Run: `./target/debug/lute compile docs/examples/haven/scenes/bridge.lute -o /tmp/haven-b.json && grep -o '"reason": *"[^"]*"' /tmp/haven-b.json`
Expected: `"reason": "bridge-reached"`

- [ ] **Step 6: Commit**

```bash
git add docs/examples/haven/scenes/bridge.lute docs/examples/haven/scenes/shed.lute
...
**Files:**
- Create: `docs/examples/haven/components/purser-interject.component.lute`
- Modify: `docs/examples/haven/scenes/cryobank.lute` (add `components:` and one `::use`)
...
```bash
git add docs/examples/haven/components docs/examples/haven/scenes/cryobank.lute
...
**Files:**
- Create: `docs/examples/haven/scenes/spine-a.lute` (episode 3)
- Create: `docs/examples/haven/scenes/hydroponics.lute` (episode 4)
- Create: `docs/examples/haven/scenes/machine-deck.lute` (episode 5)
- Create: `docs/examples/haven/scenes/stowaway.lute` (episode 6)

**Interfaces:**
- Consumes: everything from Tasks 1-6.
- Produces: `haven.s01ep06` as the join point Task 8's `spine-b` routes from.

This is a writing task: the plan fixes each scene's structural contract, and the dialogue is the deliverable produced while executing it. Every scene takes the frontmatter shape from Task 3 — `character: haven`, its episode number, `uses:` both schemas, `components:` where it invokes the Purser, and an `after:` naming its predecessor.
...
Run: `./target/debug/lute compile docs/examples/haven/scenes/spine-a.lute -o /tmp/haven-t7.json`
...
```bash
git add docs/examples/haven/scenes
...
**Files:**
- Create: `docs/examples/haven/scenes/spine-b.lute` (episode 7)
- Create: `docs/examples/haven/scenes/archive.lute` (episode 8)
- Create: `docs/examples/haven/scenes/purser.lute` (episode 9)
- Modify: `docs/examples/haven/scenes/bridge.lute` (repoint `after:`)
- Modify: `docs/examples/haven/scenes/shed.lute` (repoint `after:`)
...
`bridge.lute`'s `after:` becomes `'visited("haven.s01ep09")'`; `shed.lute`'s becomes `'visited("haven.s01ep07")'`.
...
Run: `./target/debug/lute scenario docs/examples/haven`
...
**Files:**
- Create: `docs/examples/haven/quests/unmoored.lute`
- Create: `docs/examples/haven/quests/who-wakes.lute`
- Create: `docs/examples/haven/quests/false-heading.lute`
- Create: `docs/examples/haven/quests/manifest-gap.lute`
- Create: `docs/examples/haven/quests/what-vesna-carries.lute`
...
```bash
git add docs/examples/haven/quests
...
**Files:**
- Create: `docs/examples/haven/README.md`
- Modify: `scripts/check-docs-consistency.py` (the example-roots list, if the new root needs registering — check first)
- Create: `docs/superpowers/notes/2026-07-31-haven-authoring-findings.md`

- [ ] **Step 1: Check whether the new root needs registering**

Run: `python3 scripts/check-docs-consistency.py`
It prints the example roots CI checks. If `docs/examples/haven` is covered by the existing `docs/examples` root, nothing to add — confirm rather than assume.
...
```bash
git add docs/examples/haven/README.md docs/superpowers/notes
git commit -m "docs(example): Haven README and authoring findings"
```
