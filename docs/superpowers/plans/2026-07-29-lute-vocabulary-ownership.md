...
2. Eight `crates/lute-compile/tests/snapshots/e2e__*.snap` embed a stamp: `components_scene`, `gated_line`, `quest_rescue_halsin`, `affinity_reaction`, `marina_s01ep02`, `connected_quest`, `quest_grove`, `showcase_episode01`. (`showcase_episode01` carries a *different* hash from the other seven because it folds project domains; it changes too.)
