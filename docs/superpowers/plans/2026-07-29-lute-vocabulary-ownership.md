...
```rust
    ...
    fn injected_anchor_comes_from_the_domain() {
        ...
        let (st, injected) = lower_node(StageState::default(), &show_marina_no_anchor(), &[], &doms);
    ...
    fn exit_follows_declared_exits() {
        ...
        let exit = Node::Directive(lute_syntax::ast::Directive {
            tag: "auto".into(),
            attrs: vec![attr("character", "marina"), attr("action", "vanish")],
            span: Span::default(),
        });
        let (st2, _) = lower_node(st, &exit, &[], &doms);
        assert!(!st2.on_stage.contains_key("marina"), "`vanish` must exit");
    }
```

**Note:** match the exact `Directive` literal shape the neighbouring helpers in this `mod tests` use (`attr(..)`, `Span`); read `show_marina_no_anchor` at `inject.rs:~540` and copy its construction style rather than inventing fields.
...
```rust
    ...
    fn stateful_set_has_no_unreachable_attrs() {
        for key in ["emotion", "variant", "action", "dialogMotion"] {
            assert!(
                line_is_stateful(&line("marina", vec![attr(key, "x")])),
                "`{key}` must mark a line stateful"
            );
        }
        assert!(!line_is_stateful(&line("marina", vec![attr("pose", "x")])));
    }
```
...
- Modify: `an adoption assessment (not included in this repo)` (the D1 row's `emotion` claim)
- Modify: `CHANGELOG.md`
...
| Suite | Count | Tests |
|---|---|---|
| `lute-compile/tests/e2e.rs` | 9 | `affinity_reaction`, `marina_s01ep02`, `components_scene`, `connected_quest`, `gated_line`, `quest_grove`, `quest_rescue_halsin`, `showcase_episode01`, `plugin_record_carries_resolved_effects` |
...
| `lute-compile/tests/compile.rs` | 3 | `compile_writes_out_file`, `compile_marina_exits_zero_with_artifact_json`, `compile_out_to_unwritable_path_exits_two` |
...
`an adoption assessment (not included in this repo)` — the D1 row lists `emotion` among domains a project can declare, which was false when written. Correct it and note 0.9.0 makes it true.
...
The nine: `components_scene`, `gated_line`, `quest_rescue_halsin`, `affinity_reaction`, `marina_s01ep02`, `connected_quest`, `quest_grove`, `showcase_episode01`, and `plugin_record_carries_resolved_effects` (which reads `showcase`'s `stinger.component.lute`).
...
data, and the Stage assessment's D1 row listed emotion among project-
