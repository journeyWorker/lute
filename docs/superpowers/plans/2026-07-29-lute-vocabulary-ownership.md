...
Expected: remaining `E-DOMAIN-UNKNOWN` errors name documents whose `uses:` chain does not reach `base.schema.yaml`, and the four subproject roots (`arcia-project`, `investigation`, `plugindef-project`, `showcase`) which resolve against their own `lute.project.yaml`.
...
The eight: `components_scene`, `gated_line`, `quest_rescue_halsin`, `affinity_reaction`, `marina_s01ep02`, `connected_quest`, `quest_grove`, `showcase_episode01`.
