...
- [ ] Corpus: `lute check-project docs/examples/haven` exit 0, zero new diagnostics; `compile --all` artifacts differ from pre-branch ONLY by `meta.id` + version strings (scripted JSON diff); `loc export docs/examples/haven` byte-identical to pre-branch.
