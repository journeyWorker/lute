...
assumes an *existing* TS `lute-core`/`validator` to reuse. **No such TS code exists in this repo** — the
...
- The engine never sees the DSL. Final `arcia_script_commands` emit is **out of scope** (that is
...
- The two repo examples (`marina-s01ep02.lute`, `date-minigame.lute`) serve as integration
  fixtures.
...
**Out:** final `arcia_script_commands` flat-record codegen (engine-format, engine-owned); runtime
