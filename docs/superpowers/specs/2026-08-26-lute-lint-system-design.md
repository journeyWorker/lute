...
- Port upstream's scenario lints onto the new Lute toolchain: emotion distribution &
...
This reproduces upstream's analyzer axes: run hard cap, thrash floor (streakAvg),
...
upstream's block-at-ERROR policy is reproduced per-project by setting `level: error`.
