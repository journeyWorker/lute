...
position (`s01c01ep14-hanae.lute`), which cannot express "same event, different
...
placements:
  ...
  - event: side-errand
    ...
    variants:
      - when: "run.inflow == 'hanae'"
        doc: scenes/side-errand/hanae.lute

  - event: cara-recon               # world lane, single-doc form (one unguarded variant)
    lane: world
    at: d6.evening+0
    size: 8
...
lute play <PROJECT_DIR>
  ...
  --fact  "..." ...                 # seed facts
  --script routes/hanae-a.play.yaml # route script (choices; may also carry seeds)
...
kuhen-meeting/hanae ──`), dialogue lines (speaker + emotion/action + text),
