...
The Stage onboarding scenario is moving to a Detroit: Become Human-style progression:

- The game day is divided into **8 time-of-day buckets × 12 ticks = a 96-tick clock**
  (buckets mirror the Stage wire enum `TIME_OF_DAY`).
...
1. **No scheduling surface.** Lute's `after:` graph declares causal prerequisites,
   ...
   the user lane for the hanae route".
...
3. **Scene identity is conflated with position.** The naming convention
   (`s01c01ep14-hanae.lute`: filename = timeline slot + branch arm) cannot express
...
placements:
  ...
  - event: kuhen-meeting            # event identity (matches scenes/<event>/)
    ...
    variants:                       # exactly one must be satisfiable per route
      - when: "run.inflow == 'hanae'"
        doc: scenes/kuhen-meeting/hanae.lute
      - when: "run.inflow == 'yunah'"
        doc: scenes/kuhen-meeting/yunah.lute
  - event: errand-complication
    ...
    variants:
      - when: "run.inflow == 'hanae'"
        doc: scenes/errand-complication/hanae.lute
  ...
  - event: confinement
    ...
    variants:
      - when: "run.inflow == 'hanae'"
        doc: scenes/confinement/hanae.lute
  ...
  - event: cara-recon
    ...
    at: evening+0
    size: 8
    doc: scenes/cara-recon/main.lute      # unguarded single-doc form
```
...
lute play <PROJECT_DIR>
  ...
  --script routes/hanae-a.play.yaml # choice script: hub/choice selections in order
