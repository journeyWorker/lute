...
- **Drives:** `docs/proposals/scenario-dsl/0.9.0.md` (new), `docs/proposals/plugin-system/0.0.3.md`
  ...
  `an adoption assessment (not included in this repo)` (D1 row correction), shipped JSON Schemas.
...
**Measured cost of the deviation** (`product/packages/data-catalog`, Stage):

- `emotion`: 30,861 authored values / **17 distinct**; `lute.core` admits 7, of which 6 are used.
  **6,377 occurrences (20.7%) are unrepresentable**, and core's `angry` is never used — Stage says
...
- **Directive override.** Stage's measured need is *additive* (`foreground` 430, `external` 366,
  `split-screen-*` 18, `ui` 13, `feature-*` 60 alongside 7 of core's 8). Replacing a baseline
...
and 30,861 Stage rows, bought for purity. Rejected. `irVersion` therefore stays `0.8.0`.

## D-C. Override is per-domain and declared by the replacing side

```yaml
# plugins/stage.vn/enums.yaml
...
6. `lute.vn` staying activated means non-overridden domains and all eight directives survive an
   override — the reason Stage does not have to re-declare 7 directives it uses unchanged.

**Cosmetic fix in the same change:** `merge_map` hardcodes `first: "?"` (`assemble.rs:466`), so the
current message reads ``declared by both `?` and `stage.vn` ``. Thread the owning plugin id.
...
overrides `action` (Stage: 53 members).
...
3. **Override works end to end.** The `plugins/stage.vn/` fixture used in this session's diagnosis,
   now with `overrides: [emotion]` and Stage's 17 members: `lute check scene.lute --project .`
   accepts `@marina{emotion="affection"}: hi` and still rejects `emotion="zzz"`, and
...
7. **Real-data smoke.** Generate `plugins/stage.vn/enums.yaml` from
   `product/packages/data-catalog` (emotion 17, action 53, mood 10) and `lute check-project` a scene
   converted from `arcia_script_commands/`; every previously-unrepresentable value must resolve.
...
7. Docs: `scenario-dsl/0.9.0.md`, `plugin-system/0.0.3.md`, `plugin-system.md` rationale,
   `stage-assessment.md` D1 row (its `emotion` claim is wrong today), CHANGELOG.
