...
Lute ships as an open-source authoring toolchain; Stage is one consuming application. A concrete
...
**Measured cost** (`product/packages/data-catalog`):
...
This is why Stage's 9,880 `actionId` values across 53 distinct ids get **zero** checking today: a
...
8. **Real-data smoke.** Generate a vocabulary from `product/packages/data-catalog` (emotion 17,
   action 53 with its 4 exits, vfxType 3) and `lute check-project` a scene converted from
   `arcia_script_commands/`; every previously-unrepresentable value resolves, including the 78
...
8. Docs: `scenario-dsl/0.9.0.md`, `plugin-system/0.0.3.md`, `plugin-system.md`,
   `docs/examples/*.schema.yaml`, `docs/architecture.md`, `stage-assessment.md` D1 row (its current
...
- **Directive override.** Adding directives already works via a project's own capability plugin;
  Stage's measured need is purely additive (`foreground` 430, `external` 366, `split-screen-*` 18,
  `ui` 13, `feature-*` 60 alongside 7 of core's 8 directives), and none of those rows carries
...
- **Driving reducer dispatch from `semantics` flags** (`clearsStage`, a `sceneSlot:` declaration,
  ...
  measured need is zero: no Stage-side directive wants stage rules, and `mutatesScene` cannot drive
