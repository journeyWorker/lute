...
```yaml
after: 'visited("elena.ep02") && (completed("sideQuestA") || completed("sideQuestB"))'
```
...
unrelated standalone files, and `character: marina, season: 1, episode: 1` across two distinct
...
- **Corpus grounding.** Every new diagnostic MUST be validated against the shipped
  ...
  (`character: demo`/`marina` recurring across unrelated standalone files and separate
  subprojects).
