...
- **Why:** the colon pair `:word:` is the ubiquitous emoji-shortcode convention (editors/terminals
  substitute it); the bare `:narrator:`/`:marina:` forms collide. Moving off `:` fixes it and
...
- **Mixes with valued attrs, JSX-style, space-separated:** `@marina{mono emotion="happy"}:` — a bare
