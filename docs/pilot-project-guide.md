...
- 프론트매터: `kind: scene`, `mode: inline`, `luteVersion: "0.25.0"`(= 현재 언어 버전; 낮게 스탬프하면 `W-LUTE-VERSION-STALE`), `profile: <capability profile>`. 상태 선언의 enum 스칼라는 `{ type: { enum: [...] }, default: ... }` 형태(`values:`/`domain:` 아님).
