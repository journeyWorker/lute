...
**`0.17.1` aligns all three axes at `0.17.1`, and none of them earned it.**
`0.10.1`'s and `0.15.1`'s pattern again: the release is toolchain — a
repository-hygiene rewrite that renames example cast/project identifiers to
neutral, same-length names and drops a non-normative adoption assessment — and
the language and IR numbers move because the rule says they move. Language
`0.17.1` is byte-for-byte `0.17.0` semantics
...
`lute-ir-0.19.schema.json`) and gains
...
**`0.20.0` aligns all three axes at `0.20.0`; the language earns the move.**
`check-project` gains fact envelopes: every relational query (`holds(…)`,
`count(…)`) in a guard slot is decided **impossible**, **guaranteed**, or
**possible** from a project-wide, argument-level *may* set (every ground fact
any seed, live assert, rule, or reserved relation can produce) and a
path-sensitive *must* set (the monotone facts true on every route to the slot,
within a document and across the `after:` graph). The verdict feeds the
existing decision procedure, so a dead relational guard reports through the
code its slot already owns — `E-ARM-DEAD`, `E-OBJECTIVE-UNSATISFIABLE`,
`E-QUEST-UNREACHABLE`, `W-OBJECTIVE-HIDDEN` — plus the new
`E-ENTRY-UNREACHABLE` for a lore entry `when` that can never hold; a guaranteed
query inside a guard is the new `W-FACT-GUARANTEED`. `W-UNPROVEN-RELATIONAL`
is removed, and naming it in `--deny` is a usage error, the `W-INJECT-CONFLICT`
(`0.10.0`) precedent
([`proposals/scenario-dsl/0.20.0.md`](proposals/scenario-dsl/0.20.0.md)).
No grammar changes. The IR carries no shape change — the analysis is static
...
`lute-ir-0.23.schema.json`, since renamed), gaining
...
**`0.24.0` aligns all three axes at `0.24.0`; the language and the IR both
earn the move, the IR additively.** A third dogfood round over four games (a
mystery, a roguelike, a day-clock VN, a party RPG) asked for time, quest
structure and parties
([`proposals/scenario-dsl/0.24.0.md`](proposals/scenario-dsl/0.24.0.md)). A
schema may declare one `clock:` over two `owner: engine` paths: content reads
the reserved `clock.index` / `clock.weekday` / `clock.weekdayLabel`, beats
take `once: day` / `once: slot`, and `lute play` gains `advance:` and
`include:` steps. Quests gain `activate="accept"` subquests, `complete="any"`
alternatives that fail the rest as `superseded`, a place-bound `until=`
beside a `by=` that is now judged at every settle (new
`W-DEADLINE-BEFORE-DONE` for content written for the old rule), the
read-only `quest.<id>.failedBy` / `objectives.<o>.failed`, `<on target>`,
`::accept{at="nextRun"}` and occasion `judge: before`. Entities gain
sub-kinds and entity-indexed `per:` state; a cast entry may declare
`present:` (new `W-CAST-ABSENT`) and `emotions:`; components may write state
(`effects: true`) and take `speaker` params; and `::clear`, integer `%`, a
guarded `::set{… when}`, `{{x:ordinal}}`, enum display labels, `_` in rule
bodies and `countDistinct` join the core. A plugin call's result is answered
by `bridges:` in play, test and trace, no longer by a `scene.*` seed. Guards
now narrow consistently — a beat's `when:` holds throughout its body, so an
arm it rules out is `E-ARM-DEAD` and exhaustiveness reads the same narrowed
domain — the way a 0.23-clean project can redden. The IR change is additive:
an optional `clock` on the artifact and project index, beat and entry `once`
`day` / `slot`, `ObjectiveEntry.until`, `QuestCmd.activate` / `complete`,
...
`lute-ir-0.25.schema.json`, since renamed).
Artifacts that use none of it compile byte-identically apart from the version
...
**`0.25.1` aligns all three axes at `0.25.1`, a patch on the `0.25` line; the
toolchain earns it, and the language and the IR are content no-ops (the
`0.10.1` shape).** A performance release: project commands (`check-project`,
`lore`, `scenario`, `beats`, `test`, `play`, `doctor`, `loc`,
`compile --all`) load each project's manifest, catalog and capability
snapshot once per run, parse each document once, and check documents in
parallel; a span's UTF-16 offsets come from a per-line prefix table instead
of a rescan of the file. Output, diagnostics and exit codes are
byte-identical. The language adds no syntax and no static semantics. The IR
shape does not move, so
`schemas/lute-ir-0.25.schema.json` (since renamed)
kept its name and `$id` (the `0.21.1` / `0.23.1` precedent), and artifacts
differ only in the version strings; `capabilityVersion` does not move.
Engines gate on MAJOR, so nothing widens, and the tree-sitter grammar is
unchanged.

**`0.26.0` aligns all three axes at `0.26.0`; the language and the IR both
earn the move, the IR additively.** Scale and many authors: what a lead and
four area writers asked for while writing one monster-collecting RPG in
parallel ([`proposals/scenario-dsl/0.26.0.md`](proposals/scenario-dsl/0.26.0.md)).
The fact analysis computes the seed closure once per project and `lute test`
loads a project once and runs its tests in parallel. Frontmatter `state:`
declarations of one path must agree (the new `E-STATE-DECL-CONFLICT`), a
member listed twice is `E-ENTITY-KIND-SHAPE`, a schema may `add:` members to
a kind another schema declares (a `subsetOf:` child's members count for the
parent), and `defaults.uses` entries may be globs, with `defaults.questTier`
beside them. A directive attribute may be typed by an entity kind (checked at
the `::use` a component passes it through), a reward kind's `target:`
contract is checked (the new `E-REWARD-TARGET`), and `lute refs --attr` /
`--reward` lists who gives what. Components reach parity with direct text:
their plugin result slots are declared in the host, an untyped bridge answer
is refused, `@@who:` speaks as a `speaker` param, a param may declare a
`default:`, and a component reads its own result slots. `::use`, `::accept`,
`::assert`, `::retract`, `::give` and plugin passthrough directives take
`when=` (so `when` is reserved on directives); a beat or entry may
`target="kind:<kind>"` and read the raised member as `occasion.target`; and a
rule body may `count(…)` / `countDistinct(…)` a relation its head does not
depend on (else the new `E-RULE-AGGREGATE-CYCLE`). `lute trace` and `lute test`
follow a taken `::next`, apply a handler `::accept` as play does, and fail a
test that walks an ineligible beat unless it asserts `eligible:`;
`W-DISPLAY-NAME-DUP` and `W-ENTRY-WRITE-REREAD` are new advisories, and a
compared string outside its subject's closed domain is `E-WHEN-LITERAL-DOMAIN`
on `!=` / `in` too. What moves for a 0.25-clean project: a test that walks an
ineligible beat, or expects the walk to stop at a `::next`, fails; a path
declared twice with different shapes, or a duplicate member, is an error; an
untyped bridge answer is refused; a plugin attribute named `when` is rejected
at load; and a non-member literal reports `E-WHEN-LITERAL-DOMAIN` where it
used to report `E-ARM-DEAD`. The IR change is additive: an optional
`targetKind` on beats, entries, bundle beats and index beat rows, the
placeholder kind `occasionTarget` (with its `entityKind`), and the rule-body
