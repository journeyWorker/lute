...
///
/// IR `0.11.0` is a PURE RESTAMP of `0.10.2` — no field added, renamed,
/// moved, or retyped. The `0.11.0` toolchain release is language/IR-inert
/// (`lute play` + the schedule layer, coverage, and runner fixes only), so
/// neither axis earned a content change; `docs/versioning.md`'s alignment
/// rule moves the number anyway. Unlike `0.10.1`/`0.10.2`, which shared
...
///
/// IR `0.13.0` is a PURE RESTAMP of `0.12.0` — no field added, renamed,
/// moved, or retyped. The `0.13.0` toolchain release earns it (lint system:
/// `lute lint`, `lute.lint.yaml`, plugin lints export; and `lute tag --force`
/// — a clean renumber for drafts guarded by the new `codesLocked` publish
/// gate) alongside a language change (the universal frontmatter key
/// `codesLocked`, a document-level publish latch checked and echoed but
/// never consumed by the IR). `docs/versioning.md`'s alignment rule moves
/// the IR number anyway; because the release crosses the gated major.minor
/// line (`0.12` -> `0.13`), `schemas/lute-ir-0.12.schema.json` is renamed to
/// `schemas/lute-ir-0.13.schema.json` (the `0.7.0` precedent for a
...
///
/// IR `0.23.1` is a patch on the `0.23` line with NO shape change: the
/// schema keeps its `schemas/lute-ir-0.23.schema.json` name and `$id`, and
/// documents compile byte-identically apart from the version strings, with
/// one content change: an untagged component line takes the code `lute tag`
/// would write into the component file (source order, per speaker) before a
/// param-scoped `<match>` folds, so a line in a non-first arm may get a new
/// `lineId` / `voiceKey` once — the one `lute tag` persists.
///
/// IR `0.24.0` is ADDITIVE over `0.23.1` (dsl 0.24.0): the artifact and
/// `ProjectIndex` carry an optional `clock` declaration, beat and entry
/// `once` gain `"day"` / `"slot"`, and the optional `ObjectiveEntry.until`,
/// `QuestCmd.activate` / `complete`, `OnCmd.target`, `AcceptCmd.applies`,
/// placeholder `format`, `StateEntry.labels` and the CEL binary op `%`
/// appear only when authored. Documents that use none of it compile
/// byte-identically apart from the version strings (and the capability
