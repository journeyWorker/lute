...
fn dirty_survives_join_when_only_one_arm_dirties_the_speaker() {
    ...
    // dirties marina and the other doesn't, but BOTH keep the same SpriteState,
...
<branch id="fork">
  <choice id="a" label="A">
    :line[marina]{variant="closeup"}: Hm.
  </choice>
  <choice id="b" label="B">
    :line[marina]: Yo.
  </choice>
...
fn join_unions_dirty_but_only_over_carried_characters() {
    ...
    let mut a = StageState::default();
    a.on_stage.insert("marina".into(), sprite(None));
    a.dirty.insert("marina".into());
    let mut b = StageState::default();
