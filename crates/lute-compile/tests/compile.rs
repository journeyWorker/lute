...
fn clean_doc_compiles_with_envelope_expansion_and_ids() {
    ...
    assert_eq!(artifact.lute, "0.26.0");
    assert_eq!(artifact.ir_version, "0.26.0");
...
"#;
    ...
    assert_eq!(line.line_id, "marina.ep02final.narrator_0010");
    assert_eq!(
...
"#;
    ...
    let affect = by_path("scene.affect.marina");
    assert_eq!(affect.default, Some(serde_json::json!(0)));
}
...
"#;
    ...
    assert_eq!(
        by_path("scene.affect.marina").default,
...
@marina{code="0010"}: Hi {{userName}}, {{run.coins}} left, {{@fond}}.
"#;
    let artifact = compile(&input(DOC)).expect("clean compile");
    let line = artifact
        .commands
...
"#;
    ...
    assert_eq!(v["meta"]["id"], serde_json::json!("haven.s01ep01"));

    // lineId prefix — the same `haven.s01ep01.` string, per lineId (`{prefix}.{speaker}_{code}`).
    let line = art
        .commands
        .iter()
        .find_map(|c| match c {
        ...
        .expect("a content line");
    assert!(
        line.line_id.starts_with("haven.s01ep01."),
...
"#;
    ...
    assert_eq!(actual["lute"], serde_json::json!("0.26.0"));
    assert_eq!(actual["irVersion"], serde_json::json!("0.26.0"));
}
