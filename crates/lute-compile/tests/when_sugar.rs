...
fn code_backfill_is_unaffected() {
    ...
    const ANCHOR: &str = "@elena{code=\"0010\"}: An anchor line.\n";
    ...
    assert!(
        sl[0]["lineId"].as_str().unwrap().ends_with("elena_0010"),
    ...
    assert!(
        sl[1]["lineId"].as_str().unwrap().ends_with("elena_0020"),
...
fn no_sugar_document_is_byte_identical() {
    ...
    // e2e.rs marina_s01ep02/showcase_episode01/quest_grove/
    // quest_rescue_halsin/components_scene/affinity_reaction).
    let text = scene("@elena: Hello there.\n@elena{code=\"0099\"}: Again.\n");
...
fn sugared_line_survives_compilation() {
    ...
    assert_eq!(
        pairs,
        vec![
            ("elena".to_string(), expected.clone()),
            ("elena".to_string(), expected),
        ],
...
fn component_gated_line_when_binds_param_before_fold() {
    ...
    assert_eq!(fond_lines[0]["speaker"], "marina");
