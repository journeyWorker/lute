...
/// Warning-bearing golden: `marina-s01ep02.lute` is error-clean but carries a
...
fn headless_and_lsp_diagnostics_match_warning_bearing() {
    ...
    assert!(
        !res.diagnostics.is_empty(),
        "marina-s01ep02.lute must produce diagnostics; an empty vector would make the golden trivially pass"
    );
    assert!(
        res.diagnostics.iter().any(|d| d.severity == Severity::Warning),
        "marina-s01ep02.lute should carry at least one warning-severity diagnostic (covers the Warning round-trip)"
    );
...
/// once the `arcia.minigame` plugin is resolved through the shared project
...
fn divergence_holds_under_plugin_project() {
    ...
    // `providerRef` id `marina_service_01` resolves and positions match.
    let providers = ProviderSet::load("../../docs/examples/arcia-project/catalog");
    // The scene's own `uses:` chain, resolved by the SAME `resolve_imports` both
    // surfaces call. `arcia.minigame` ships directives and a bridge but no
    ...
    let dir = std::path::Path::new("../../docs/examples/arcia-project");
...
fn divergence_holds_under_inline_enums() {
    ...
    let bad = "---\nkind: scene\ncharacter: demo\nseason: 1\nepisode: 1\n\
               enums:\n  emotion: [neutral, gleeful]\n---\n## Shot 1.\n\
               @marina{emotion=\"zzz\"}: not a declared member.\n";
