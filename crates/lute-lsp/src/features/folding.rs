...
mod tests {
    ...
    /// marina example the block opens at `<timeline duration="1.4">` and closes at
    ...
    fn timeline_fold_spans_full_block() {
        ...
        let open = MARINA.find("<timeline duration").unwrap();
        let close = MARINA.find("</timeline>").unwrap();
