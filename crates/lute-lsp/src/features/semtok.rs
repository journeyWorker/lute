...
use lute_syntax::ast::{Arm, ClipNode, Directive, Document, Node, Quest, Set};
...
mod tests {
    ...
    fn camera_directive_tag_is_staging() {
        ...
        let cam = MARINA.find("::camera{").unwrap();
        let p = idx.position(cam);
        let (want_line, want_ch) = (p.line - 1, p.utf16_col);
        let decoded = decode(&tokens(MARINA));
        let tok = decoded
            .iter()
