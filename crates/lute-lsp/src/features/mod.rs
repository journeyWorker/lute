...
/// `scene.affect.marina` does not match `scene.affect.marina_2`.
...
mod tests {
    ...
    fn path_tokens_skips_dotted_text_inside_cel_string() {
        ...
        let toks = path_tokens("scene.affect.marina == 'scene.affect.marina'");
        let count = toks
            .iter()
            .filter(|(t, _)| t == "scene.affect.marina")
            .count();
        assert_eq!(
            count, 1,
    ...
    fn references_ignore_path_inside_cel_string_literal() {
        ...
        // dotted text inside the `<when test="'scene.affect.marina' == 'x'">`
