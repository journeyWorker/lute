...
mod tests {
    ...
    /// A Marina-shaped fixture: a `<branch id="number">` plus a `<match on=…>` on
    ...
    fn definition_on_choices_path_jumps_to_branch() {
        let doc = parsed(MARINA);
        // Cursor inside the `on="scene.choices.number"` subject path.
        let off = MARINA.find("scene.choices.number").unwrap() + 5;
        let loc =
            definition_at(&doc, &load_core_snapshot(), &SchemaImports::default(), off).unwrap();
        let branch_line = line_of(MARINA, MARINA.find("<branch id=").unwrap());
        assert_eq!(line_of(MARINA, loc.byte_start), branch_line);
    }

    #[test]
    fn definition_on_ref_jumps_to_def_decl() {
        let doc = parsed(MARINA);
        let off = MARINA.find("@fond").unwrap() + 1;
        let loc =
            definition_at(&doc, &load_core_snapshot(), &SchemaImports::default(), off).unwrap();
        let def_line = line_of(MARINA, MARINA.find("  fond:").unwrap() + 2);
        assert_eq!(line_of(MARINA, loc.byte_start), def_line);
    }

    #[test]
    fn definition_on_state_path_jumps_to_decl() {
        let doc = parsed(MARINA);
        // The `::set{scene.affect.marina += 1}` target path.
        let set_at = MARINA.find("::set{").unwrap();
        let off = MARINA[set_at..].find("scene.affect.marina").unwrap() + set_at + 2;
        let loc =
            definition_at(&doc, &load_core_snapshot(), &SchemaImports::default(), off).unwrap();
        let decl_line = line_of(MARINA, MARINA.find("  scene.affect.marina:").unwrap() + 2);
        assert_eq!(line_of(MARINA, loc.byte_start), decl_line);
    }
    ...
    fn references_on_state_path_counts_set_and_reads() {
        ...
        let off = text[set_at..].find("scene.affect.marina").unwrap() + set_at + 2;
        let refs = references_at(
            &doc,
            &load_core_snapshot(),
            &SchemaImports::default(),
    ...
    fn nav_asset_segment_none() {
        ...
        let doc = parsed(text);
        let off = text.find("marina").unwrap() + 1;
