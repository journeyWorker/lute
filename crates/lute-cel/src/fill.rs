...
mod tests {
    ...
    #[test]
    fn marina_example_fills_all_slots_without_errors() {
        let text = include_str!("../../../docs/examples/marina-s01ep02.lute");
        let (mut doc, _) = parse(text);
        let mut arena = CelArena::default();
        let errs = fill_document(&mut arena, &mut doc);
        assert!(errs.is_empty(), "marina CEL is all valid: {errs:?}");
