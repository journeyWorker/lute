...
fn scene_uses_enum_schema_checks_clean_and_domain_is_merged() {
    ...
    let imports = resolve_imports(&dir, &["schema.lute".to_string()], &[], zero_span());
    assert!(
        imports.diags.is_empty(),
        "unexpected import diags: {:?}",
        imports.diags
    );
    ...
    let (merged, diags) = merge_domains(&input.snapshot, &imports, no_inline(), zero_span());
...
fn open_domain_accepts_any_string() {
    ...
    assert!(
        codes.is_empty(),
        "open domain must always-accept, got {codes:?}"
    );
}
...
fn closed_domain_membership_wins_over_same_named_provider() {
    ...
    assert!(diags.is_empty(), "unexpected merge diags: {diags:?}");
    assert_eq!(
        merged["action"].members,
        vec!["wave".to_string(), "bow".to_string()]
    );
    ...
    let ok_codes = codes_with_domain_attr_against("{ domain: action }", "wave", &snapshot, &merged);
    assert!(
        !ok_codes
            .iter()
    ...
    let bad_codes = codes_with_domain_attr_against("{ domain: action }", "zzz", &snapshot, &merged);
    assert!(
...
const INLINE_FIXTURE: &str = "---\nkind: scene\ncharacter: demo\nseason: 1\nepisode: 1\n\
     ...
     @marina{emotion=\"gleeful\"}: declared INLINE in this very document.\n";
...
fn inline_long_form_enum_carries_the_slot_member_semantics() {
    ...
    let ok = scene_codes(
        &dir,
        &scene(long_action, "@marina{action=\"wave\"}: line."),
        load_core_snapshot(),
    );
    assert!(
        ok.is_empty(),
    ...
    let anchor_ok = scene_codes(
        &dir,
        &scene(long_anchor, "@marina: line."),
        load_core_snapshot(),
    );
    assert!(
...
fn inline_and_imported_enum_is_a_d5_refinement_never_domain_dup() {
    ...
    let grow = scene_codes(
        ...
        &scene(
            ...
            "@marina{emotion=\"feral\"}: inline grows the imported list.",
        ),
        load_core_snapshot(),
    );
    assert!(
    ...
    let shrink = scene_codes(
        ...
        &scene(
            "uses: vocab.lute\nenums:\n  emotion: [neutral]\n",
            "@marina{emotion=\"neutral\"}: inline drops a base member.",
        ),
        load_core_snapshot(),
    );
    assert!(
    ...
    assert_eq!(
        shrink
            .iter()
            .filter(|c| *c == "E-EXTENDS-RELATION-SIG")
            .count(),
        1,
...
fn inline_enum_clashing_with_a_plugin_domain_is_domain_dup() {
    ...
    let snapshot = lute_test_vocab::vocab_snapshot();
    assert!(
        !snapshot.domains["emotion"]
            .members
            .contains(&"gleeful".to_string()),
    ...
    let codes = scene_codes(
        ...
        &scene(
            "enums:\n  emotion: [neutral, gleeful]\n",
            "@marina{emotion=\"gleeful\"}: shadowing a plugin domain.",
        ),
        snapshot,
    );
    assert!(
...
fn inline_and_imported_declarations_check_identically() {
    ...
    for (label, inline_front, schema) in routes {
        ...
        let slot = if inline_front.contains("action") {
            "action"
        } else {
            "emotion"
        };
        for value in ["gleeful", "zzz"] {
            let body = format!("@marina{{{slot}=\"{value}\"}}: line.");
            let dir = unique_dir();
...
/// dsl 0.24.0 §1: an enum's `labels:` keys are members — a label for a
/// non-member is `E-ENUM-LABEL-NOT-MEMBER` on the inline AND the imported
/// route (the shared `validate_domain` rules) — and each label is a string
/// (`E-META-VALUE`, the frontmatter value-shape code).
#[test]
fn enum_labels_must_name_members_and_be_strings() {
