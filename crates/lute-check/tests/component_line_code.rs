...
const DUP_BODY: &str = "@marina{code=\"0010\"}: one\n@marina{code=\"0010\"}: two\n";
...
fn dup_line_code_three_way_parity() {
    for (label, diags) in [
        ...
        assert!(
            ...
            "the duplicate (marina, 0010) pair must report E-DUP-LINE-CODE at {label}, got {:?}",
            codes(&diags)
        );
    }
}
...
fn distinct_pairs_in_component_body_stay_clean() {
    let clean = "@marina{code=\"0010\"}: one\n@marina{code=\"0020\"}: two\n\
    ...
    let trimmed = "@marina{code=\"0010\"}: one\n@marina{code=\" 0010 \"}: two\n";
    assert!(
...
fn each_component_body_is_its_own_scope() {
    ...
    assert!(
        !codes(&diags).contains(&"E-DUP-LINE-CODE".to_string()),
        "two components each using (marina, 0010) once must not collide, got {:?}",
        codes(&diags)
    );
}
