...
#[test]
fn marina_style_fond_bool_def_is_clean() {
    // Mirrors the real `docs/examples/marina-s01ep02.lute` def
    // `fond: { type: bool, cel: "scene.affect.marina >= 1" }`, used only in a
    ...
    assert!(
        !codes(&t).contains(&"E-REF-TYPE".to_string()),
        "marina's `fond: bool` in a bool guard must NOT flag E-REF-TYPE; got {:?}",
        codes(&t)
    );
}
