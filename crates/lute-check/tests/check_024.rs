...
fn staged_character_outside_the_cast_is_unknown_with_a_suggestion() {
    ...
    let anchored: Vec<&str> = hits
        .iter()
        .map(|d| &src[d.span.byte_start..d.span.byte_end])
        .collect();
    ...
    assert!(
        hits[0].message.contains("did you mean `mara`"),
        "{}",
        hits[0].message
    );
    assert!(
        hits[1].message.contains("`::camera{focus}` `tomsa`"),
        "{}",
        hits[1].message
    );
    assert!(
        hits[1].message.contains("did you mean `tomas`"),
        "{}",
        hits[1].message
    );
}
...
fn a_handler_whose_when_implies_completion_never_runs() {
    ...
    assert!(
        hits[0].message.contains("quest `hunt`"),
        "{}",
        hits[0].message
    );
    ...
    assert!(
        handler_dead(&quest("", &two, on)).is_empty(),
        "`all` needs both objectives"
    );
}
...
fn a_handler_that_can_run_while_active_is_clean() {
    let on = |when: &str| {
        format!("<on event=\"bossDefeated\" when=\"{when}\">\n@narrator: Saw it.\n</on>")
    };
