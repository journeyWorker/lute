...
fn project_beat_diags(files: &[(&str, &str)]) -> Vec<(PathBuf, Diagnostic)> {
    ...
    let foldeds: Vec<FoldedEnv> =
        files.iter().zip(&docs).map(|((_, t), (_, d))| fold_env(d, &input(t)).0).collect();
...
fn share_needs_a_written_spending_once() {
    ...
    for text in [
        ...
        assert!(
            ds.iter().any(|d| d.message.contains("`share` `solWarm` without `once`")),
            "{text}\n{ds:#?}"
        );
    }
    // A key is an identifier.
    let bad = lore("<beat id=\"radio\" on=\"visit\" once=\"user\" share=\"sol warm\">\n@n: a\n</beat>\n");
    assert!(beat_attr(&bad).iter().any(|d| d.message.contains("must be a key")), "{:#?}", beat_attr(&bad));
    // Scene-only frontmatter: a beat key, so `share:` needs `on:` too.
    let no_on = beat_attr(&scene("a.one", "once: user\nshare: solWarm\n"));
    assert!(no_on.iter().any(|d| d.message.contains("`share:` without `on:`")), "{no_on:#?}");
}
...
fn every_beat_of_a_key_declares_the_same_once() {
    ...
    let ds: Vec<&Diagnostic> = out.iter().map(|(_, d)| d).filter(|d| d.code == "E-BEAT-ATTR").collect();
    assert_eq!(ds.len(), 1, "{out:#?}");
    assert!(
        ds[0].message.contains("beat `talks.roof` shares `solWarm` with beat `talks.radio`"),
        "{}",
        ds[0].message
    );
    // Across documents and kinds, one `once` is one spend.
    let talks = lore("<beat id=\"radio\" on=\"visit\" once=\"user\" share=\"solWarm\">\n@n: a\n</beat>\n");
...
fn the_presence_ladder_reads_a_shared_spend_as_the_keys_disjunction() {
    ...
    let rungs: Vec<&String> = ladder[&PathBuf::from("talks.lute")].values().flatten().collect();
    assert_eq!(rungs, ["(visited('talks.radio') || visited('talks.roof'))"], "{ladder:#?}");
}
...
fn bundle_after_draws_a_scenario_edge() {
    assert!(check(&input(WITNESSES)).ok, "{:#?}", check(&input(WITNESSES)).diagnostics);
    ...
    assert!(graph.edges.get(&from).is_some_and(|t| t.contains(&to)), "{:#?}", graph.edges);
    assert_eq!(
        graph.edge_kinds_for(&from, &to).map(|k| k.iter().copied().collect::<Vec<_>>()),
...
fn bundle_after_is_checked_like_a_scene_after() {
    let bad = WITNESSES.replace("after=\"visited('witnesses.maren')\"", "after=\"!visited('witnesses.maren')\"");
    let r = check(&input(&bad));
    assert!(r.diagnostics.iter().any(|d| d.code == "E-CONN-PROFILE"), "{:#?}", r.diagnostics);
    ...
    assert!(
        out.iter()
            .any(|(_, d)| d.code == "E-CONN-UNKNOWN-NODE" && d.message.contains("did you mean `witnesses.maren`")),
        "{out:#?}"
    );
}
...
fn a_visited_when_conjunct_is_an_unanchored_hint() {
    ...
    assert_eq!(
        ...
        [(NodeId::Beat("witnesses.marenConfronted".into()), vec!["witnesses.maren".to_string()])]
    );
