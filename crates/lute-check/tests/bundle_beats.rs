...
use lute_check::{check, check_project_beats, fold_env, CheckInput, CheckResult, Mode, SchemaImports};
...
fn with_code(text: &str, code: &str) -> Vec<Diagnostic> {
    run(text).diagnostics.into_iter().filter(|d| d.code == code).collect()
}
...
const PORTER: &str = "<beat id=\"porter\" on=\"talk\" target=\"npc.porter\" when=\"run.trust >= 0\">\n\
     @porter: You again.\n\
     <branch id=\"talk\">\n\
...
fn beat_shape_faults_are_beat_attr() {
    for (open, needle) in [
        ...
        ("<beat id=\"a\" on=\"talk\" priority=\"high\">", "must be an integer"),
        ("<beat id=\"a\" on=\"talk\" once=\"never\">", "`once=\"never\"`"),
        ("<beat id=\"a\" on=\"talk\" also=\"maybe\">", "is a flag"),
        ("<beat id=\"a\" on=\"talk\" target=\"npc..x\">", "malformed"),
    ] {
        let ds = with_code(&doc(&format!("{open}\n@n: hi\n</beat>\n")), "E-BEAT-ATTR");
        assert!(ds.iter().any(|d| d.message.contains(needle)), "{open}: {ds:#?}");
    }
    let ds = with_code(
        "---\nkind: lore\n---\n<beat id=\"a\" on=\"talk\">\n@n: hi\n</beat>\n",
        "E-BEAT-ATTR",
    );
    assert!(ds.iter().any(|d| d.message.contains("document `id:`")), "{ds:#?}");
    let dup = doc("<beat id=\"a\" on=\"talk\">\n@n: 1\n</beat>\n<beat id=\"a\" on=\"talk\">\n@n: 2\n</beat>\n");
    assert!(with_code(&dup, "E-BEAT-ATTR").iter().any(|d| d.message.contains("duplicate")));
...
fn beat_when_follows_the_scene_beat_rules() {
    let dead = doc("<beat id=\"a\" on=\"talk\" when=\"run.trust > 5 && run.trust < 3\">\n@n: hi\n</beat>\n");
...
fn an_always_eligible_repeatable_beat_shadows_a_later_one() {
    ...
    let shadowed: Vec<_> = out.iter().filter(|(_, d)| d.code == "W-BEAT-SHADOWED").collect();
...
fn visited_resolves_a_bundle_beat_in_a_condition_and_in_after() {
    ...
    let docs = project(&[("i.lute", &lore), ("n.lute", when), ("l.lute", after), ("t.lute", typo)]);
