...
fn fold_diag_in_component_body_reported_once() {
    ...
    assert!(
        d.message
            .contains("`marina` is shown without an explicit `anchor`"),
...
fn scene_level_fold_diag_is_not_duplicated() {
    ...
    // Root site on `kenshi`, body site on `marina`: two distinct sites, one
    // diagnostic each.
    let scene = "---\nkind: scene\ncharacter: x\nseason: 1\nepisode: 1\n\
                 components: [c.lute]\n---\n## Shot 1.\n\
...
/// twice reports ONCE — the first invocation stages `marina`, so the second
...
fn a_component_body_diagnostic_carries_its_internal_position() {
    ...
    // `@marina{emotion="smug"}: Hello.`, and `smug` opens at column 18).
