...
fn inject_conflict_in_component_body_reported_once() {
    ...
    assert!(
        d.message.contains("`marina` is shown with an explicit `anchor=\"center\"`"),
...
fn scene_level_conflict_is_not_duplicated() {
    ...
    // Root conflict on `kenshi`, body conflict on `marina`: two distinct sites,
    // one warning each.
    let scene = "---\nkind: scene\ncharacter: x\nseason: 1\nepisode: 1\n\
                 components: [c.lute]\n---\n## Shot 1.\n\
                 ::auto{character=\"kenshi\" anchor=\"center\"}\n\
                 ::use{component=\"c\"}\n";
    ...
    assert!(
        cs[0].message.contains("`kenshi`") && cs[1].message.contains("`marina`"),
        "each names its own character: {cs:#?}"
    );
}

/// The seam, pinned: the body folds against the stage state INHERITED at the
/// `::use` site. `marina` is already on stage when the `::use` runs, so
...
fn use_site_inherits_stage_state_no_invented_conflict() {
    let scene = "---\nkind: scene\ncharacter: x\nseason: 1\nepisode: 1\n\
                 components: [c.lute]\n---\n## Shot 1.\n\
                 ::auto{character=\"marina\" anchor=\"left\"}\n\
                 ::use{component=\"c\"}\n";
    let diags = check_with_components(
        scene,
        &[(
...
/// component twice warns ONCE — the first invocation stages `marina`, so the
