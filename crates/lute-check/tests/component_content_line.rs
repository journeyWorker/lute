...
const PARITY_LINES: &[&str] = &[
    ...
    "@marina{emotion=\"neutral\" variant=\"a\" vo}: hi",
];
...
fn unknown_attr_in_component_body() {
    let cs = component_codes("@marina{pose=\"idle\"}: hi", vocab_snapshot());
    assert!(
...
fn bad_enum_in_component_body() {
    ...
    let clean = component_codes("@marina{emotion=\"neutral\"}: hi", vocab_snapshot());
    assert!(
...
fn delivery_rules_in_component_body() {
    ...
    let conflict = component_codes("@marina{mono os vo}: hi", vocab_snapshot());
    assert_eq!(
        conflict
            .iter()
...
fn stamp_attr_is_admitted_in_component_body() {
    ...
    let cs = component_codes("@marina{beat=\"b1\"}: hi", snap.clone());
    assert!(
        !cs.contains(&"E-UNKNOWN-ATTR".to_string()),
        "a declared cross-cutting stampAttr must not be E-UNKNOWN-ATTR in a component body: {cs:?}"
    );
    assert_eq!(
        scene_codes("@marina{beat=\"b1\"}: hi", snap),
        cs,
