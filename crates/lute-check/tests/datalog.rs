...
fn derive_without_rules_is_warning() {
    ...
    assert!(
        c.contains(&"W-DERIVE-NO-RULES".to_string()),
        "orphan + canReach have no rules: {c:?}"
    );
}
...
fn unbound_head_var_is_unsafe() {
    ...
    assert!(
        c.contains(&"E-DATALOG-UNSAFE".to_string()),
        "B unbound: {c:?}"
    );
}
...
fn kind_is_a_unary_domain_predicate() {
    ...
    assert!(
        ...
        "the spec §7 ally rule is clean: {c:?}"
    );
}
...
fn head_const_is_domain_checked() {
    ...
    assert!(
        ...
        "grove is a location, not a faction (D6): {c:?}"
    );
}
...
fn now_in_guard_is_guard_fact_and_clean_scalar_guard_is_fine() {
    let now = codes(&scene_with(&format!(
...
fn guard_taint_propagates_to_downstream_readers() {
    ...
    assert!(
        tainted.contains("downstream"),
        "downstream reads guarded, so it is tainted too (§6): {tainted:?}"
    );
    assert!(
        !tainted.contains("clean"),
        "clean never reads a guarded relation: {tainted:?}"
    );
}
