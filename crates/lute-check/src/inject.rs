...
mod tests {
    ...
    // --- brief helpers ---
    fn show_marina_no_anchor() -> Node {
        auto(vec![attr("character", "marina")])
    }
    fn line_marina() -> Node {
        line("marina", vec![])
    }
    ...
    fn show_without_anchor_injects_anchor_with_provenance() {
        ...
        let (st2, injected) = lower_node(st, &show_marina_no_anchor(), &[], &doms);
        assert!(injected
            .iter()
    ...
    fn dirty_pose_before_nonstateful_line_injects_posreset() {
        ...
        let (st2, injected) = lower_node(st, &line_marina(), &[], &anchor_domain("center"));
        assert!(injected
            .iter()
            .any(|c| c.provenance.by == "auto-pose-reset"));
        assert!(!st2.dirty.contains("marina"), "dirty flag should clear");
    }
    ...
    fn stateful_line_does_not_pose_reset_and_marks_dirty() {
        ...
        let (st2, injected) = lower_node(
            st,
            &line("marina", vec![attr("emotion", "delighted")]),
            &[],
        ...
        assert!(!injected
            .iter()
            .any(|c| c.provenance.by == "auto-pose-reset"));
        assert!(st2.dirty.contains("marina"), "a stateful line marks dirty");
    ...
    fn stateful_set_has_no_unreachable_attrs() {
        ...
        let spoken = |attrs: Vec<Attr>| match line("marina", attrs) {
            Node::Line(l) => l,
    ...
    fn entry_emotion_lookahead_preloads_first_emotion() {
        let st = StageState::default();
        let look = [line("marina", vec![attr("emotion", "delighted")])];
        let (st2, injected) = lower_node(
            st,
            &show_marina_no_anchor(),
            &look,
            &anchor_domain("center"),
        );
        let load = injected
    ...
    fn explicit_default_anchor_warns_inject_conflict() {
        ...
        assert_eq!(st2.on_stage["marina"].anchor.as_deref(), Some("center"));
    }
    ...
    fn explicit_override_anchor_is_silent() {
        ...
        // no conflict.
        let st = StageState::default();
        let show = auto(vec![attr("character", "marina"), attr("anchor", "left")]);
        let (st2, injected) = lower_node(st, &show, &[], &anchor_domain("center"));
        assert!(!injected
            .iter()
            .any(|c| c.provenance.by == "auto-anchor-on-show"));
        assert!(st2.diags.is_empty());
        assert_eq!(st2.on_stage["marina"].anchor.as_deref(), Some("left"));
    }
    ...
    fn reducer_is_pure_same_inputs_same_outputs() {
        ...
        let look = [line("marina", vec![attr("emotion", "sad")])];
        let doms = anchor_domain("center");
        let (a_st, a_em) = lower_node(build(), &show_marina_no_anchor(), &look, &doms);
        let (b_st, b_em) = lower_node(build(), &show_marina_no_anchor(), &look, &doms);
        assert_eq!(a_em, b_em);
    ...
    fn exit_follows_declared_exits() {
        ...
        let mut st = StageState::default();
        st.on_stage.insert("marina".into(), SpriteState::default());
        let exit = auto(vec![attr("character", "marina"), attr("action", "vanish")]);
    ...
    fn no_declared_anchor_domain_injects_nothing() {
        ...
        assert_eq!(st.on_stage["marina"].anchor, None);
    }
}
