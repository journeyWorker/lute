...
mod tests {
    ...
    fn dirty_pose_before_nonstateful_line_injects_posreset() {
        ...
        let (st2, injected) = lower_node(st, &line_marina(), &[]);
    ...
    fn stateful_line_does_not_pose_reset_and_marks_dirty() {
        ...
        st.on_stage.insert("marina".into(), SpriteState::default());
        let (st2, injected) =
            lower_node(st, &line("marina", vec![attr("emotion", "delighted")]), &[]);
        assert!(!injected.iter().any(|c| c.provenance.by == "auto-pose-reset"));
    ...
    fn entry_emotion_lookahead_preloads_first_emotion() {
        ...
        let (st2, injected) = lower_node(st, &show_marina_no_anchor(), &look);
        let load = injected
            .iter()
    ...
    fn explicit_default_anchor_warns_inject_conflict() {
        let st = StageState::default();
        let show = auto(vec![attr("character", "marina"), attr("anchor", DEFAULT_ANCHOR)]);
        ...
        assert_eq!(st2.on_stage["marina"].anchor.as_deref(), Some(DEFAULT_ANCHOR));
    }
    ...
    fn exit_action_frees_the_stage_slot() {
        ...
        st.on_stage.insert("marina".into(), SpriteState::default());
        st.dirty.insert("marina".into());
        let exit = auto(vec![attr("character", "marina"), attr("action", "fade-out-down")]);
    ...
    fn reducer_is_pure_same_inputs_same_outputs() {
        ...
        let (a_st, a_em) = lower_node(build(), &show_marina_no_anchor(), &look);
        let (b_st, b_em) = lower_node(build(), &show_marina_no_anchor(), &look);
        assert_eq!(a_em, b_em);
