...
fn haven() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/examples/haven")
}
...
fn envelope_reports_the_relational_layer_not_only_the_scalar_one() {
    let root = haven();
    let text = stdout(&run(&["scenario", root.to_str().unwrap(), "envelope", "haven.s01ep06"]));
...
fn envelope_at_the_root_and_at_the_leaf_are_no_longer_identical() {
    let root = haven();
    let first = stdout(&run(&["scenario", root.to_str().unwrap(), "envelope", "haven.s01ep01"]));
    let last = stdout(&run(&["scenario", root.to_str().unwrap(), "envelope", "haven.s01ep09"]));
    let strip = |s: &str| s.replace("haven.s01ep01", "N").replace("haven.s01ep09", "N");
    ...
    // have named scene(haven.s01ep09), eight scenes later, as a writer of a
    // path readable on arrival at haven.s01ep01.
    let head = |s: &str| {
    ...
    assert!(
        !head(&first).contains("written by: scene(haven.s01ep09)"),
