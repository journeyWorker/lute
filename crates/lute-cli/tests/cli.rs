...
fn check_file_with_errors_exits_one() {
    let out = Command::new(BIN)
        .args(["check", "../../docs/examples/arcia-project/date-minigame.lute", "--json"])
        .output()
        .unwrap();
    assert!(
        !out.status.success(),
...
fn check_human_output_lists_diagnostics() {
    let out = Command::new(BIN)
        .args(["check", "../../docs/examples/arcia-project/date-minigame.lute"])
        .output()
        .unwrap();
    assert_eq!(out.status.code(), Some(1));
...
fn check_with_empty_providers_dir_is_permissive() {
    ...
    let out = Command::new(BIN)
        .args([
            "check",
            "../../docs/examples/marina-s01ep02.lute",
            "--providers",
        ])
        .arg(&dir)
        .arg("--json")
...
fn hub_demo_example_checks_clean() {
    ...
    assert!(
        preview.contains("scene.choices.chatWithMarina"),
...
fn context_core_only_has_eight_core_directives() {
    ...
    let out = Command::new(BIN)
        .args(["context", "../../docs/examples/marina-s01ep02.lute", "--json"])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
...
fn context_choice_slot_domain_includes_unset() {
    ...
    // members (no spurious `unset`). `hub-demo` folds the hub `chatWithMarina` into
    ...
    assert_eq!(
        domain_of("scene.choices.chatWithMarina"),
...
fn context_human_lists_delivery_flags() {
    let out = Command::new(BIN)
        .args(["context", "../../docs/examples/marina-s01ep02.lute"])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
