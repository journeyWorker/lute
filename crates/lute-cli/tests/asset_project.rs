//! Authored `assetId` acceptance (plugin §6.9): the on-disk `arcia.minigame`
...
//! scene with a VALID authored id (`MG.marina_service_01.0`) checks clean; a bad
...
fn mgart_bad_segment_flags() {
    // A temp scene identical to arcia-portrait.lute EXCEPT an absent minigameId
    ...
    let scene = tmp.join("arcia-portrait-bad.lute");
    std::fs::write(
        &scene,
        "---\n\
         character: marina\n\
