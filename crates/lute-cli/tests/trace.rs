...
fn trace_marks_an_exiting_auto_as_an_exit() {
    let out = trace(&["../../docs/examples/haven/scenes/wake.lute", "--project", "../../docs/examples/haven"]);
...
fn trace_renders_the_end_reason_and_reports_a_disposition() {
    let out = trace(&["../../docs/examples/haven/scenes/bridge.lute", "--project", "../../docs/examples/haven"]);
    let text = String::from_utf8_lossy(&out.stdout).to_string();
...
fn trace_prints_the_shot_heading_it_is_holding() {
    let out = trace(&["../../docs/examples/haven/scenes/hydroponics.lute", "--project", "../../docs/examples/haven"]);
    let text = String::from_utf8_lossy(&out.stdout).to_string();
