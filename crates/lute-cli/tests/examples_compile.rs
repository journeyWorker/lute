...
// `<otherwise>` block-open tag (e.g. `<when test="X">@elena: text`) parses
...
// each carry a `@elena:` line. Before 27653b6 both arms' dialogue was written
...
// carry a `@elena:` line, under a `--project` resolution (declaration
...
fn extends_demo_example_compiles_with_all_when_otherwise_arm_lines_present() {
    ...
    assert_eq!(
        pairs,
        owned_pairs(&[
            ("elena", "Fortune smiles on me today."),
            ("elena", "Still waiting on my luck to turn."),
        ]),
