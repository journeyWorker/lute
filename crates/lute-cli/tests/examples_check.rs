...
fn corpus_check_project_is_clean_end_to_end() {
    ...
    // docs/examples itself, showcase/, plugindef-project/, arcia-project/,
...
fn corpus_no_false_positive_episode_dup_across_or_within_project_roots() {
    ...
    // marina` recurs in docs/examples/, arcia-project/, AND showcase/ --
    ...
    // components/scene.lute, param-def.lute), two `elena.s01ep01`
    // (choice-persist.lute, gated-line.lute), and two `marina.s01ep05`
    // inside the arcia-project root (date-minigame.lute,
    // arcia-portrait.lute) -- genuine TRUE positives this grounding test
    ...
    // gated-line.lute 1->5, arcia-portrait.lute 5->6) -- content/behavior
...
/// directory layout -- `showcase/`, `arcia-project/`, `plugindef-project/`
...
fn shipped_scene_files() -> Vec<(&'static str, PathBuf)> {
    ...
    let arcia = root.join("arcia-project");
    ...
    vec![
        ...
        ("marina-s01ep02.lute", root.clone()),
        ...
        ("arcia-project/date-minigame.lute", arcia.clone()),
        ("arcia-project/arcia-portrait.lute", arcia),
