...
fn world_lane_interleaves_between_errand_and_the_rewind_and_is_hidden_by_default() {
    ...
    assert!(!default_text.contains("cara"), "world scene must be hidden under default --lanes user: {default_text}");
...
fn coverage_reports_full_corpus_covered_and_exits_zero() {
    ...
    write(&dir, "routes/a-right.play.yaml", "state:\n  run.route: a\nchoose:\n  meet/pick: right\n");
    let out = run(&[
        "play",
        dir.to_str().unwrap(),
        "--coverage",
...
fn coverage_json_output_reports_the_uncovered_option() {
    ...
    let opts = v["options"]["uncovered"].as_array().expect("uncovered options array");
    assert!(
        opts.iter().any(|o| o["event"] == "meet" && o["id"] == "pick" && o["missing"][0] == "right"),
        "{v}"
    );
}
...
fn each_scene_resets_scene_state_from_its_own_declared_default() {
    ...
    assert!(!text.contains("scene-b sees calm."), "scene-b must reset from ITS OWN default: {text}");
}
...
fn optional_placement_unsatisfied_at_seed_is_reconsidered_after_an_earlier_scene_sets_state() {
    ...
    assert!(text.contains("hidden optional fires."), "hidden-optional must be reconsidered live: {text}");
}
...
fn a_placement_that_switches_variant_presents_at_the_live_variants_own_position() {
    ...
    assert!(!text.contains("movable sees no."), "the live-active variant must be `yes`, never `no`: {text}");
...
fn unresolved_variant_guard_halts_incomplete_naming_the_surface() {
    ...
    assert!(!text.contains("should never print."), "the guard must never silently decide false: {text}");
    assert!(!text.contains("E-SCHED-VARIANT-GAP"), "an unresolved guard is incomplete, never a mundane gap: {text}");
}
...
fn world_placement_switching_variant_mid_drain_is_rescanned_and_fires() {
    ...
    assert!(text.contains("mover sees B."), "mover must be rescanned onto its live `B` variant: {text}");
    assert!(!text.contains("mover sees A."), "mover's seed-time `A` variant must never fire once B is live: {text}");
}
...
fn world_variant_gap_is_deferred_until_the_placement_is_actually_due() {
    ...
    assert!(text.contains("late event fires."), "late-event's gap check must defer until its own boundary: {text}");
    ...
    assert!(scene_two_pos < late_pos, "late-event must drain after the scene that unlocks it: {text}");
}
...
fn hub_sequence_exhausted_with_eligible_options_remaining_halts_incomplete() {
    ...
    assert!(text.contains("leave"), "the still-eligible option must be named: {text}");
    assert!(!text.contains("after hub."), "the walk must not fall through past an exhausted hub: {text}");
}
