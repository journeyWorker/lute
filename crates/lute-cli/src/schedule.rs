...
struct RawPlacement {
    ...
    #[serde(default)]
    optional: bool,
    /// Single unguarded-variant shorthand (spec §3's `cara-recon` example) —
...
mod tests {
    ...
    fn load_schedule_single_doc_shorthand_normalizes_to_one_unguarded_variant() {
        ...
        write_scene(&dir, "scenes/cara-recon/main.lute");
        write_schedule(
            &dir,
            &format!(
                "{YAML_HEADER}placements:\n  - event: cara-recon\n    lane: world\n    at: morning+0\n    size: 4\n    doc: scenes/cara-recon/main.lute\n"
            ),
        );
        ...
        assert_eq!(v.doc, "scenes/cara-recon/main.lute");
    ...
    fn cursor_dynamic_rejected_after_route_dependent_predecessor() {
        ...
        write_scene(&dir, "scenes/a/hanae.lute");
        write_scene(&dir, "scenes/a/yunah.lute");
        write_scene(&dir, "scenes/b/x.lute");
        write_schedule(
            &dir,
            &format!(
                "{YAML_HEADER}placements:\n\
                 \x20 - event: a\n    lane: user\n    at: morning+0\n    size: 4\n    variants:\n\
                 \x20     - when: \"run.inflow == 'hanae'\"\n        doc: scenes/a/hanae.lute\n\
                 \x20     - when: \"run.inflow == 'yunah'\"\n        doc: scenes/a/yunah.lute\n        at: afternoon+0\n        size: 6\n\
                 \x20 - event: b\n    lane: user\n    size: 3\n    doc: scenes/b/x.lute\n"
            ),
        );
        let (sched, diags) = load_schedule(&dir).unwrap().unwrap();
    ...
    fn inflow_enums() -> BTreeMap<String, Vec<String>> {
        let mut m = BTreeMap::new();
        m.insert("run.inflow".to_string(), vec!["hanae".to_string(), "yunah".to_string()]);
        m
    }

    #[test]
    fn route_space_variant_gap_detected() {
        // only hanae covered — yunah route has zero satisfiable variants.
        let variants = vec![variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 4)];
        ...
        assert!(gap.message.contains("yunah"), "{}", gap.message);
    }
    ...
    fn route_space_variant_ambig_detected() {
        // both variants satisfiable simultaneously when inflow == hanae
        let variants = vec![
            variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 4),
    ...
    fn route_space_no_gap_or_ambig_when_exhaustive_and_exclusive() {
        let variants = vec![
            variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 4),
            variant("scenes/a/yunah.lute", Some("run.inflow == 'yunah'"), Some(0), 4),
        ];
        let s = sched_with(vec![placement("a", "user", 0, Some(0), 4, false, variants)], lanes_user_exclusive());
        let diags = route_space_check(&s, &inflow_enums());
        assert!(find_diag(&diags, E_SCHED_VARIANT_GAP).is_none(), "{diags:?}");
    ...
    fn route_space_user_overlap_detected_for_co_satisfiable_overlapping_placements() {
        let va = vec![variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 10)];
        let vb = vec![variant("scenes/b/hanae.lute", Some("run.inflow == 'hanae'"), Some(5), 10)]; // overlaps [0,10) with [5,15)
        let s = sched_with(
    ...
    fn route_space_no_overlap_when_mutually_exclusive_routes() {
        let va = vec![variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 10)];
        let vb = vec![variant("scenes/b/yunah.lute", Some("run.inflow == 'yunah'"), Some(5), 10)]; // ticks overlap but never co-satisfiable
        let s = sched_with(
            vec![placement("a", "user", 0, Some(0), 10, true, va), placement("b", "user", 1, Some(5), 10, true, vb)],
            lanes_user_exclusive(),
        );
    ...
    fn route_space_no_overlap_on_non_exclusive_lane() {
        let va = vec![variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 10)];
        let vb = vec![variant("scenes/b/hanae.lute", Some("run.inflow == 'hanae'"), Some(5), 10)];
        let s = sched_with(
    ...
    fn assume_prunes_sentinel_domain_member_from_gap_sweep() {
        // domain {none, hanae, yunah}; variants cover hanae+yunah only. Without
        // the assumption, `none` fires a GAP; with it, the sweep is clean.
        let mut enums = BTreeMap::new();
        enums.insert(
            "run.inflow".to_string(),
            vec!["none".to_string(), "hanae".to_string(), "yunah".to_string()],
        );
        let variants = vec![
            variant("scenes/a/hanae.lute", Some("run.inflow == 'hanae'"), Some(0), 4),
            variant("scenes/a/yunah.lute", Some("run.inflow == 'yunah'"), Some(0), 4),
    ...
    fn assume_never_hides_a_real_gap() {
        // yunah is inside the assumed route space and uncovered — GAP stays.
        let mut enums = BTreeMap::new();
        enums.insert(
            "run.inflow".to_string(),
            vec!["none".to_string(), "hanae".to_string(), "yunah".to_string()],
        ...
        let diags = route_space_check(&s, &enums);
        let gap = find_diag(&diags, E_SCHED_VARIANT_GAP).expect("yunah gap must survive");
        assert!(gap.message.contains("yunah"), "{gap:?}");
    }
    ...
    fn assume_malformed_cel_reported_and_ignored() {
        ...
        let diags = route_space_check(&s, &inflow_enums());
        assert!(find_diag(&diags, E_SCHED_GUARD_PARSE).is_some(), "{diags:?}");
        // a broken assumption must not prune anything: yunah's gap still fires.
