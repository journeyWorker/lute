...
fn assemble_merges_asset_kinds() {
    let mut pkg = plugin_with_directive("arcia.minigame", "minigame");
        ...
        let hit = errs
            ...
            .find(|e| matches!(
                ...
                lute_manifest::assemble::AssembleError::ReservedStampAttr { plugin, name }
                    if name == reserved && plugin == "arcia.minigame"
            ))
            .unwrap_or_else(|| {
...
fn assemble_rejects_reserved_names_in_stamp_attrs_export() {
    for reserved in [
        ...
        let mut p = plugin_with_directive("arcia.minigame", "minigame");
        p.stamp_attrs = vec![
            AttrDecl {
                name: reserved.into(),
                required: false,
...
fn stamp_attrs_merge_and_participate_in_capability_version() {
    let base = plugin_with_directive("arcia.minigame", "minigame");
    ...
    assert!(
        errs.iter().any(|e| matches!(
            ...
            lute_manifest::assemble::AssembleError::UnknownRewardKindTarget { plugin, kind, provider }
                if plugin == "arcia.minigame" && kind == "ITEM" && provider == "item"
        )),
...
fn assemble_reward_kind_resolves_peer_declared_provider() {
    let mut kind_pkg = plugin_with_directive("arcia.rewards", "rewards");
    kind_pkg
        .reward_kinds
        .push(reward_kind("ITEM", Some("item")));
    let mut prov_pkg = plugin_with_directive("arcia.items", "items");
    prov_pkg.providers.push(provider("item"));
    let reg = InstalledPlugins {
        by_id: BTreeMap::from([
            (
                "arcia.rewards".to_string(),
                InstalledPlugin { loaded: kind_pkg },
            ),
            (
                "arcia.items".to_string(),
