...
fn option_validation_reads_the_manifest_of_the_active_plugin() {
    ...
    assert_eq!(
        errs[0].message(),
        "option `arcia.vn.resultScope` expects enum(scene|run), got \"galaxy\""
    );
}
...
fn resolver_surfaces_option_and_lowering_diagnostics_end_to_end() {
    ...
    let pdir = root.join("plugins/arcia.vn/directives");
