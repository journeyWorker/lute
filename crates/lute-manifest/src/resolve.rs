...
mod tests {
    ...
    fn resolves_extends_chain_parent_first_with_core_and_global() {
        ...
        assert_eq!(
            ids,
            vec!["lute.core", "arcia.vn", "arcia.date", "arcia.minigame"]
        );
    }
    ...
    fn dependency_closure_pulls_transitive_deps() {
        ...
        let graph = ProfileGraph {
            profiles: BTreeMap::from([(
                ...
                Profile {
                    extends: None,
                    plugins: BTreeMap::from([("arcia.vn".to_string(), BTreeMap::new())]),
                },
            )]),
    ...
    fn opt_manifest() -> crate::schema::PluginManifest {
        let mut m = manifest("arcia.minigame", "0.1.0", &[]);
        m.options = vec![
    ...
    fn one_plugin_graph(opts: BTreeMap<String, Literal>) -> ProfileGraph {
        ProfileGraph {
            profiles: BTreeMap::from([(
                ...
                Profile {
                    extends: None,
                    plugins: BTreeMap::from([("arcia.minigame".to_string(), opts)]),
                },
            )]),
            default_profile: "story".to_string(),
        }
    ...
    fn unknown_option_name_is_rejected() {
        ...
        assert_eq!(
            errs[0].message(),
            "plugin `arcia.minigame` has no option `resultScop` (declared: resultScope, rounds)"
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    ...
                    "option `arcia.minigame.resultScope` expects enum(scene|run), got \"galaxy\""
                        .to_string()
                ),
                (
                    "E-PLUGIN-OPTION-TYPE",
