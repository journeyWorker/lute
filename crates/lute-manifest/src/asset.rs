...
mod tests {
    ...
    fn fresh_providers() -> ProviderSet {
        ProviderSet::from_one(ProviderSnapshot {
            ...
            entries: [("character".to_string(), vec!["marina".to_string()])]
                .into_iter()
                .collect(),
            stale: false,
        })
    ...
    #[test]
    fn decompose_ch_ok() {
        let got = decompose(&ch_kind(), "CH.marina.waitress.delighted.1").unwrap();
    ...
    fn decompose_const_mismatch() {
        assert_eq!(
            decompose(&ch_kind(), "XX.marina.waitress.delighted.1"),
    ...
    fn validate_ok() {
        let ch = ch_kind();
        let segs = decompose(&ch, "CH.marina.waitress.delighted.1").unwrap();
        assert_eq!(
    ...
    fn validate_bad_enum() {
        let ch = ch_kind();
        let segs = decompose(&ch, "CH.marina.waitress.badmood.1").unwrap();
        assert_eq!(
            validate_segments(&ch, &segs, &fresh_providers()),
    ...
    fn validate_not_number() {
        let ch = ch_kind();
        let segs = decompose(&ch, "CH.marina.waitress.delighted.x").unwrap();
        assert_eq!(
            validate_segments(&ch, &segs, &fresh_providers()),
