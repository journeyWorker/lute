...
mod tests {
    ...
    fn comment_not_recognized_inside_content_text() {
        ...
        let input = "@marina: I love /* c */ and // b";
        let out = strip_comments_checked(input).unwrap();
    ...
    fn leading_comment_before_content_line_keeps_text_opaque() {
        ...
        let input = "/* pre */ @marina: keep /* c */ here";
        let out = strip_comments_checked(input).unwrap();
        assert!(!out.contains("pre"), "leading comment survived: {out:?}");
        assert!(out.contains("@marina:"), "construct lost: {out:?}");
        assert!(
    ...
    fn escaped_backslash_attr_value_keeps_text_opaque() {
        ...
        let input = r#"@marina{u="\\"}: keep /* literal */"#;
        let out = strip_comments_checked(input).unwrap();
