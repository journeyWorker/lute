...
fn line_without_when_is_none() {
    ...
    let doc = parse_body(r#"@elena: plain line, no guard."#);
    let line = first_line(&doc);
...
fn walk_visits_the_when_slot() {
    let doc = parse_body(r#"@elena{when="run.metHelpfully"}: Guarded."#);
...
fn slot_order_is_stable() {
    ...
    let doc = parse_body(r#"@elena{mood=@moodRef focus=@focusRef}: no guard here."#);
    let mut raws = Vec::new();
    for_each_cel_slot(&doc, &mut |slot| raws.push(slot.raw.clone()));
