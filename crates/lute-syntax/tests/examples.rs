//! Integration test: the worked marina example must parse cleanly (dsl §4.3, §7).

use lute_syntax::ast::{Arm, ClipNode, Node, TrackKey};

const MARINA: &str = "../../docs/examples/marina-s01ep02.lute";
...
#[test]
fn marina_block_assembly_is_correct() {
    let text = std::fs::read_to_string(MARINA).unwrap();
    ...
    assert_eq!(doc.shots.iter().map(|s| s.number).collect::<Vec<_>>(), [Some(1), Some(2), Some(3), Some(4), Some(5)]);

    // Shot 3: <timeline duration="1.4"> with 4 tracks; the beam clip lands at 0.5.
    let timeline = doc.shots[2]
        .body
        .iter()
        .find_map(|n| if let Node::Timeline(t) = n { Some(t) } else { None })
        .expect("shot 3 has a timeline");
    assert_eq!(timeline.duration.as_ref().map(|c| c.raw.as_str()), Some("1.4"));
    ...
    if let ClipNode::Directive(d) = &timeline.tracks[0].clips[1].node {
        assert!(d.attrs.iter().all(|a| a.key != "at"), "at must move to Clip.at");
    } else {
    ...
    let m = doc.shots[4]
        .body
        .iter()
        .find_map(|n| if let Node::Match(m) = n { Some(m) } else { None })
