...
Lute is the scenario-authoring language and compiler surface for visual-novel episodes. This repository starts with the language design documents extracted from Upstream so the DSL can evolve as its own project instead of being hidden inside the broader Upstream workspace.
...
- [`docs/examples/marina-s01ep02.lute`](docs/examples/marina-s01ep02.lute) — worked example showing comments, camera directives, timeline tracks, branching, matching, and state.
...
<match on="scene.minigame.service01.rank">
  <when test="$ == 'gold'">
    :line[marina]{code="0200"}: Wonderful!
  </when>
  <otherwise>
    :line[marina]{code="0210"}: Shall we try again?
  </otherwise>
</match>
```
