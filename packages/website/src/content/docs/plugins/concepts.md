...
The [character/cast](https://github.com/KantoRegion/lute/blob/main/docs/proposals/character-cast/0.0.1.md) capability is the canonical example: it adds the `cast:` frontmatter key, a `character` provider, `scene.cast.<id>.*` state, the `CH` asset kind, and the `::seal` / `::reveal` / `::wear` directives — all pure desugaring to `::set`, no new semantics.
