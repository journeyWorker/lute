...
description: How Lute plugins extend vocabulary and capability surfaces without ever touching the fixed grammar or compiler behavior, and how every declaration they make is validated rather than trusted.
---
...
- **Enums, defs, frontmatter keys, asset kinds, world events, and cross-cutting attributes** — closed value sets, shared typed-CEL `@refs`, plugin-owned `---` meta keys, structured asset-id templates, `<on event>` triggers, and [`stampAttrs`](/plugins/manifests/) admissible on every directive and content line.
...
## A declaration is checked, not trusted

Declaring a surface in a manifest does not make the toolchain believe it. Every plugin-owned surface is validated before an author can reach it: [option values at activation](/plugins/profiles/) (`E-PLUGIN-OPTION-UNKNOWN`, `E-PLUGIN-OPTION-TYPE`), [attribute names against the core stamp](/plugins/manifests/) (`E-PLUGIN-RESERVED-STAMP-ATTR`), and [declarative lowering against the target record's real field table](/plugins/manifests/) (`E-LOWER-RECORD-UNKNOWN`, `E-LOWER-RECORD-FIELD`). All of them fail at load, assembly, or activation — before a broken manifest can mislead a writer.
...
A value with no manifest-literal representation at all — `null`, a `!tag`, a non-string mapping key — is the same error, never a panic and never a silent pass. Core frontmatter keys (the universal set, the scene-only set, and the component-only set) keep their existing bespoke handling and are not routed through this check.
