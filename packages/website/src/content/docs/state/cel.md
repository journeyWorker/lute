...
- quest `<quest start="…" fail="…">` and `<objective done="…" when="…" by="…">` predicates, `<on when="…">` handlers
...
A condition reads declared state paths, and the reserved paths the engine writes without any declaration: the quest slots `quest.<id>.state`, `quest.<id>.activatedAt`, and `quest.<id>.objectives.<o>.done`, the lore entry flags `entry.<id>.read` (run tier) and `entry.<id>.everRead` (user tier, since 0.22.0), and, since 0.23.0, `prev.run.<path>`, the value each declared `run.<path>` had when the previous run ended (always maybe-unset; see [The previous run](/state/state-model/#the-previous-run)). None of them is ever a `::set` target — see [Paths the engine writes](/state/state-model/#paths-the-engine-writes).
