...
compiles, traces, and inspects them. The current language version is **0.18.0**.
...
```
$ lute version --json
{"toolchain":"0.18.0","language":"0.18.0","ir":"0.18.0"}
```

(`lute --version` also works and prints just `lute 0.18.0` — the toolchain axis alone.)
