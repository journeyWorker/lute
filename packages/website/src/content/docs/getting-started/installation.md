...
compiles, traces, and inspects them. The current language version is **0.19.0**.
...
```
$ lute version --json
{"toolchain":"0.19.0","language":"0.19.0","ir":"0.19.0"}
```

(`lute --version` also works and prints just `lute 0.19.0` — the toolchain axis alone.)
