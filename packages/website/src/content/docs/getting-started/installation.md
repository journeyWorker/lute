...
compiles, traces, and inspects them. The current language version is **0.15.1**.
...
```
$ lute version --json
{"toolchain":"0.15.1","language":"0.15.1","ir":"0.15.1"}
```

(`lute --version` also works and prints just `lute 0.15.1` — the toolchain axis alone.)
