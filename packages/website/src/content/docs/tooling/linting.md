...
A plugin lists `lints: lints/` under `exports:` in its `plugin.yaml` and ships
`lints/*.yaml` files, each a top-level `lints:` list of the same rule shape used by
`custom`:
