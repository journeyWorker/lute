...
Entries have no `once`: re-presentation is their nature. An entry that should be heard once guards on its own `entry.<id>.read`.
...
| `1` | Error — the project fails to compile, a vocabulary conflict, or a `pick` that is not eligible. |
| `2` | Usage or I/O — a bad script, an unknown occasion, an unreadable project, a malformed artifact. |
| `3` | Incomplete — an unscripted choice or hub, a `when` or quest objective that evaluates to unknown, or an unresolved `now()` / `validAt()` / plugin `bridgeResult`. |
