...
description: The consolidated index of what the Lute language enforces today at version 0.17.0 — each language area mapped to the versioned proposal that introduced or last changed it, all pointing back to the normative repository sources.
---
...
the consolidated **index** of what is *current* at language version **0.17.0**:
...
- **`0.11.0` moves the IR's `major.minor` with nothing behind it — the first
  ...
  to `schemas/lute-ir-0.17.schema.json` under the `0.7.0` precedent — a
