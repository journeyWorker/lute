...
| Lore entries (queried content) | 0.19.0 (`kind: lore` documents of top-level `<entry>` declarations — content the engine looks up rather than plays; `id` / `target` / `category` / `title` / `series` / `order` / `when` attributes, `target` and `category` shape-only; bodies admit content lines, `<match>`, `::set`, `::assert`, `::retract`; knowledge revealed by plain `::assert`; the reserved engine-written `entry.<id>.read` path; first-read-only effects; a document-level `series:` ordering entries by file position; new `E-ENTRY-ATTR`, `E-ENTRY-ID-DUP`, `E-ENTRY-SERIES-ORDER`) | 0.21.0 (an entry answers an engine occasion with `on=` / `priority=` beside its `when` / `target`, making it an entry beat; entries take no `once` — re-presentation is their nature) | [0.21.0.md](https://github.com/journeyWorker/lute/blob/main/docs/proposals/scenario-dsl/0.21.0.md) |
...
- **`0.11.0` moves the IR's `major.minor` with nothing behind it — the first
  ...
  to `schemas/lute-ir-0.21.schema.json` under the `0.7.0` precedent — a
