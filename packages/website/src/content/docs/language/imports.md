...
<match on="run.choseHelp">
  <when test="$ == true"> @elena: Thanks for helping me back then. </when>
  <otherwise>             @elena: ... </otherwise>
</match>
```

*(From [`docs/examples/carry-ep.lute`](https://github.com/org/lute/blob/main/docs/examples/carry-ep.lute).)* The scene reads `run.choseHelp`, which is declared in the imported
...
*(From [`docs/examples/extends-demo.lute`](https://github.com/org/lute/blob/main/docs/examples/extends-demo.lute), whose `child.schema.yaml` itself `extends:` a base — the DAG runs several files deep.)*
