...
The [`docs/examples/showcase/`](https://github.com/org/lute/tree/main/docs/examples/showcase) project is one self-contained scenario that exercises **every implemented Lute feature** end-to-end and checks clean:
...
<match on="scene.serve.debut.rank">
  ...
  <otherwise>
    @marina{code="0040" emotion="shy" variant="0"}: Shall we try once more?
  </otherwise>
</match>
```
...
For the complete project — the plugin manifests, schemas, component, catalog, and the full feature→line map — see the [showcase directory and its README](https://github.com/org/lute/tree/main/docs/examples/showcase) in the repository.
