---
title: 치트시트
description: "Lute 0.22.0로 글을 쓰는 동안 열어 두는 한 페이지: 모든 구문을 검사된 최소 스니펫으로 보여 줍니다(프로젝트 구성, 프론트매터, 대사, 선택지, match, 상태, CEL, 비트, 퀘스트, 로어, 컴포넌트, 타임라인). CLI 요약, 작가가 가장 자주 만나는 진단 코드, 주의할 점도 담았습니다."
---
...
정렬됩니다. `select: first`는 맨 위의 비트를 제시하고, `select: all`은 플레이어가 고르게 합니다. 대상이
같거나 둘 다 없고 priority도 같은 `select: first` 비트 둘의 `when`이 서로 배타적임을 증명할 수 없으면
승자는 파일 순서로 정해집니다(`check-project`의 `W-BEAT-PRIORITY-TIE`). 계기와 월드 이벤트는 플러그인이
선언합니다:
...
| `<on event>` | `questActive`, `questComplete`, `questFailed`, 또는 플러그인의 월드 이벤트. `when=`으로 가드할 수 있습니다. 결코 실패할 수 없는 퀘스트(`fail`도, 필수 하위 퀘스트 목표도, 부모 퀘스트도 없음)의 `questFailed` 핸들러는 `W-QUEST-HANDLER-DEAD`입니다. |
...
| `lute check-project <dir>` | 모든 문서와 함께 연결성, 퀘스트 id, `::accept` 대상, 계기, 팩트 가드를 검사합니다. 깨끗한 문서는 모두 컴파일까지 해 보므로, 컴파일 단계 오류(`E-DUP-VOICEKEY`, `E-CAPABILITY-MISMATCH`)도 여기서 실패합니다. `W-BEAT-PRIORITY-TIE`, `W-QUEST-HANDLER-DEAD` 같은 프로젝트 권고도 여기서 나옵니다. |
...
| `lute trace <file> [--mock m.yaml] [--state P=V] [--fact "r(a)"] [--choose id=c[,c]] [--event e] [--accept q] [--occasion o] [--entry id] [--no-derive]` | 프로젝트의 시드 팩트와 규칙을 적용한 채 소스를 목에 맞춰 미리 봅니다. 종료 코드 `3`은 판정할 수 없는 가드를 만났다는 뜻입니다. |
...
| `lute context <file> [--project <dir>]` | 여기서 쓸 수 있는 모든 것: 디렉티브(내장 포함), 어휘, 상태(`owner: engine` 표시), def, 등급과 `reserved` 여부를 담은 관계, 대상 도메인을 담은 계기, 컴포넌트 시그니처, 모든 씬·퀘스트·엔트리 id. |
...
- `target`, `pick`, `choose`, `expect`는 `occasion` 스텝에만 쓰고, `label`과 `repeat`은 어느 스텝에나 쓸 수
  ...
  `notOffered`를 받습니다. 어긋나면 그 스텝과 실제 값을 알려 줍니다.
...
| `E-BEAT-ATTR` | 비트 키 형식이 잘못되었거나 `on`이 없거나, `when`이 `scene.*`를 읽거나, 대상 없는 계기에 `target`을 썼거나 대상이 대상 도메인 밖에 있거나(비슷한 이름 제안과 함께), 엔트리의 `once`가 `run`이나 `user`가 아닙니다. |
...
| `W-QUEST-HANDLER-DEAD` | 결코 실패할 수 없는 퀘스트(`fail`도, 필수 하위 퀘스트 목표도, 부모 퀘스트도 없음)에 `<on event="questFailed">`가 있습니다. |
