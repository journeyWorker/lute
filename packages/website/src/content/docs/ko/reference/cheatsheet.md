---
title: 치트시트
description: "Lute 0.21.1로 글을 쓰는 동안 열어 두는 한 페이지: 모든 구문을 검사된 최소 스니펫으로 보여 줍니다(프로젝트 구성, 프론트매터, 대사, 선택지, match, 상태, CEL, 비트, 퀘스트, 로어, 컴포넌트, 타임라인). CLI 요약, 작가가 가장 자주 만나는 진단 코드, 주의할 점도 담았습니다."
---
...
| `user.*` | 프로필 초기화 시 | 예 |
| `app.*` | 앱 삭제 시 | 아니요 |
| `quest.<id>.state`(항상 값이 있음: 퀘스트가 활성화되기 전에는 `unset`, 그 뒤로 `active` `complete` `failed`), `quest.<id>.activatedAt`, `quest.<id>.objectives.<o>.done` | 엔진 | 아니요 |
...
| `when` | `run` / `user` / `app`, `quest.*`, `entry.*.read`, 팩트, `visited()`에 대한 CEL. `scene.*`는 읽을 수 없습니다. 문자열을 비교할 때는 YAML 값을 큰따옴표로 감싸 CEL이 작은따옴표를 쓸 수 있게 하세요: `when: "run.slot == 'night' && user.runs >= 3"`. 두 층 모두 작은따옴표를 쓰면 `E-META-PARSE`입니다. |
...
| `<reward kind amount target when on/>` | 엔진에 넘기는 데이터입니다. 콘텐츠는 보상을 읽을 수 없고 `lute run` / `lute play`는 `grant`만 출력하므로, 같은 재화를 `<on>` 핸들러에서 `::set`으로 또 올리지 마세요. 두 번 지급됩니다. `amount`는 정수나 범위 `N..M`입니다. `on="failed"`는 실패 시에 지급합니다. |
...
파라미터만 렌더링하며 `string` 파라미터는 렌더링하지 않습니다. 가져오는 씬은 `components:`에 파일을 적고
...
| `lute check-project <dir>` | 모든 문서와 함께 연결성, 퀘스트 id, `::accept` 대상, 계기, 팩트 가드를 검사합니다. 깨끗한 문서는 모두 컴파일까지 해 보므로, 컴파일 단계 오류(`E-DUP-VOICEKEY`, `E-CAPABILITY-MISMATCH`)도 여기서 실패합니다. |
...
| `lute test [<dir>] [--project <dir>] [--coverage]` | 모든 `*.test.yaml`을 실행합니다. 미완료로 끝난 워크는 실패합니다. `--coverage`는 `--project`나 가장 가까운 `lute.project.yaml`의 프로젝트에서 테스트되지 않은 문서를 나열합니다. |
...
| `lute lint [<path>] [--config lute.lint.yaml]` | 프로젝트별로 설정하는 권고성 편집 린트(`L-*`). |
...
테스트의 `file:`은 씬이나 퀘스트 문서여야 합니다. 로어 문서는 `E-TEST-LORE`입니다. 엔트리는
`lute trace <file> --entry <id>`로 미리 보세요.
...
| `E-DUP-VOICEKEY` | 텍스트가 다른 줄들이 같은 `voiceKey`로 컴파일됩니다. `identity.voiceKey: "{prefix}.{speaker}-{code}"`를 설정하세요. |
| `E-CAPABILITY-MISMATCH` | 프로젝트의 문서들이 서로 다른 기능 스냅샷으로 해석되어(다른 프로필이나 씬별 `plugins:`) 하나로 컴파일할 수 없습니다. |
| `E-TEST-LORE` | `*.test.yaml`이 워크가 없는 로어 문서를 가리킵니다. `lute trace <file> --entry <id>`를 쓰세요. |
...
엔트리에는 `once`가 없습니다. `when="!entry.<id>.read"`로 가드한 엔트리는 **런마다** 한 번 들립니다.
`entry.<id>.read`는 run 등급이라 새 런에서 초기화되기 때문입니다. 평생 한 번이라면 엔트리가 세우는
`user.*` 플래그로 가드하거나, `once: user` 씬으로 만드세요.
...
**`trace`와 `test`는 명시한 세계에서 시작하며 도출하지 않습니다.** 스키마의 `facts:` 시드는 불러오지
않으니 `--fact`나 `facts:`로 넘기세요. 목으로 주지 않은 기본 팩트는 거짓입니다. 목으로 주지 않은
`derive: true` 팩트는 그 규칙에 필요한 팩트를 다 주었더라도 unknown이며 워크를 멈춥니다(종료 코드 3).
도출된 팩트 자체를 목으로 주세요. `lute test`에서는 이렇게 멈추면 `expect: { exit: incomplete }`를
선언하지 않은 테스트는 실패합니다. `lute run`과 `lute play`는 시드와 Datalog 규칙을 적용합니다.
