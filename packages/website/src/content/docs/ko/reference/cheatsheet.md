---
title: 치트시트
description: "Lute 0.23.1과 dsl 0.24.0 추가 사항(시계, 퀘스트 구조, 파티, 플레이와 테스트의 브리지 응답)으로 글을 쓰는 동안 열어 두는 한 페이지: 모든 구문을 검사된 최소 스니펫으로 보여 줍니다(프로젝트 구성, 프론트매터, 대사, 선택지, match, 상태, CEL, 비트, 시계, 퀘스트, 로어, 컴포넌트, 타임라인). CLI 요약, 작가가 가장 자주 만나는 진단 코드, 주의할 점도 담았습니다."
---
